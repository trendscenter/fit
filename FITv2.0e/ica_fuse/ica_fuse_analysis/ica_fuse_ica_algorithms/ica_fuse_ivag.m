function [W, cost, Sigma_N, isi] = ica_fuse_ivag(X,varargin)
% [W,cost,Sigma_N,isi] = ica_fuse_ivag(X,varargin)
%
...
%
% Example call:
% W=iva_G(X,'opt_approach','newton','circular',true)
%
...
% function.
%
% To test code call function with no input arguments: iva_G
%
% Coded by Matthew Anderson (matt.anderson at umbc.edu)
%
% References:
%
% [1] M. Anderson, T. Adali, & X.-L. Li, "Joint Blind Source Separation of Multivariate Gaussian Sources: Algorithms and Performance Analysis," IEEE Trans. Signal Process., 2012, 60, 1672-1683
...
function test_iva_G
%% built-in test function for iva_G
...
W=iva_G(x,'jdiag_initW',false,'opt_approach','newton');
% show results
T1 = zeros(N,N);
for k = 1 : K
    Tk = W(:,:,k)*A(:,:,k);
...
% %% Example calls to iva_G with various options
% % note that the inputs could be made into a cell array with K entries
% W=iva_G(X,'jdiag_initW',false,'opt_approach','newton');
% [~,isi]=bss_isi(W,A,S);
% disp(['Newton IVA-G ISI: ' num2str(isi)])
W=iva_G(x,'A',A,'jdiag_initW',false,'opt_approach','gradient','whiten',true);
[~,isi]=bss_isi(W,A,S);
...
%    X_cell{k}=X(:,:,k);
% end
% W_cell=iva_G(X_cell,'jdiag_initW',false,'opt_approach','newton');
% for k=1:K
%    W(:,:,k)=W_cell{k};
% end
% [~,isi]=bss_isi(W,A,S);
...
% %% Example calls to iva_G with various options
% % note that the inputs could be made into a cell array with K entries
% W=iva_G(X,'A',A,'jdiag_initW',false,'opt_approach','newton','circular',true);
% [~,isi]=bss_isi(W,A,S);
% disp(['Circular IVA-G ISI: ' num2str(isi)])
% W=iva_G(X,'A',A,'jdiag_initW',false,'opt_approach','newton');
% [~,isi]=bss_isi(W,A,S);
