function ica_fuse_plotNextPreviousExitButtons(graphicsH)
...
for numFig = numFigures:-1:1
    ...
    BackButton = ica_fuse_uicontrol('parent', graphicsH(numFig).H, 'style', 'pushbutton', 'units', ...
    ...
    ExitButton = ica_fuse_uicontrol('parent', graphicsH(numFig).H, 'style', 'pushbutton', 'units', 'normalized',...
        'position', ExitButtonPos, 'string', 'X', 'callback', {@exitCallback, graphicsH(numFig).H});
    % next button and position
    NextButtonPos =[ExitButtonPos(1) + ExitButtonPos(3) + xOffset BackButtonPos(2) buttonWidth buttonHeight];
    NextButton = ica_fuse_uicontrol('parent', graphicsH(numFig).H, 'style', 'pushbutton', 'units', 'normalized', ...
