...
while step(1) < maxsteps || step(2) < maxsteps || step(3) < maxsteps
    ...
    A1 = WtoA(weights{1},dewhiteM{3});%%%%%%%%%%time cource matrix of data1 (fmri)
    A2 = WtoA(weights{2},dewhiteM{1});%%%%%%%%%%A matrix of data2 (smri)
    A3 = WtoA(weights{3},dewhiteM{2});%%%%%%%%%A matrix of data3 (dmri)
    num_IC = size(A1,2);
    num_sub = size(A2,1);
    num_vox_fmri = size(data1,2);
    %  build C1 : num_sub*num_IC (distance^2) matrix of data2 (fmri) basesd on distance^2 between Sagg
    %  and Ssub
    S_group = weights{1}*data1;
    back_reconstructed_s_fmri_sub_stacked = whiteM{5};
    for sub = 1:num_sub%%calculate source for all subjects, ie back reconstruction
        ...
        S_sub(sub,:,:) = weights{1}*temp_whiteM; 
    end
    ...
    [idx,simt]=find_mean_v2(C1,A2,A3,MaxComCon); %give me the MaxComCon most corr
    idxt{ii}=idx;
    sim {ii}=simt;
    %% -----------------------------------------------------------
    % Calculate variable for the most correlated triplets
    for trip = 1:MaxComCon
        [sim_c_out,dA_corr1_out,dA_corr2_out,dA_corr3_out,alphak_out] = pre_calc_links_deltas(C1,A2,A3,Crate,idx,trip,Connect_threshold,alphak(trip),S_sub,S_group,num_sub,num_IC,num_vox_fmri,data1);
        alphak(trip) = alphak_out;	% alphak is related to the step length on the optimization search
...
%         weights{1} = AtoW(A1,whiteM{1});
    ...
    if ~stopsign(2) %&& mod(iters,3)==1
        ...
        %% ************* update A2 smri based on gradient *****************
        A2 = WtoA(weights{2},dewhiteM{1});
        A2(:,idx(:,2))=A2(:,idx(:,2)) + dA_corr2;
        weights{2} = AtoW(A2,whiteM{1});
    ...
    if ~stopsign(3) %&& mod(iters,3)==2
        ...
        %% ************* update A3 based on gradient *****************
        A3 = WtoA(oldweights{3},dewhiteM{2});
        A3(:,idx(:,3))=A3(:,idx(:,3)) + dA_corr3;
        weights{3} = AtoW(A3,whiteM{2});
...
% changed by Shile Qi (move 1 to 2 & move 1 to 3)
function [sim_d,dA_corr1,dA_corr2,dA_corr3,alphak] = pre_calc_links_deltas(inA1,inA2,inA3,inCrate,inidx,inCons_com,Connect_threshold,inalphak,S_sub,S_group,num_sub,num_IC,num_vox_fmri,data1)
...
tempWZ = 2*tempDj*data1';
%move 1 to 2
deltaf=(comterm12*( (a2'-mean(a2'))' + b12/var(a1')*(mean(a1') -a1')'))*tempWZ;% 1st order derivative
P=deltaf./norm(deltaf) ;% (H2*deltaf')';
aweights_temp12=inalphak*P;

%move 1 to 3
deltaf=comterm13*( (a3'-mean(a3'))' + b13/var(a1')*(mean(a1')-a1')')*tempWZ;% 1st order derivative
