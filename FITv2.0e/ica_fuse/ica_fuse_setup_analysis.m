function fusionFile = ica_fuse_setup_analysis(inputFile, fusionFile)
...
[inputText] = ica_fuse_define_parameters(inputFile);
...
if isfield(fusionInfo.setup_analysis, typePCATag)
    ...
    typePCAVal = strmatch(lower(typePCAStr), lower(cellstr(get(typePCAHandle, 'string'))), 'exact');
...
try
    ...
    if (strcmpi(typePCA, 'mccar') && (numFeatures == 1))
        error('MCCAR requires more than one feature');
    end
