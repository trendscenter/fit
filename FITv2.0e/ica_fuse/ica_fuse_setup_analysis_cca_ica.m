function fusionFile = ica_fuse_setup_analysis_cca_ica(inputFile, fusionFile)
...
inputText = ica_fuse_define_parameters_cca_ica(inputFile);
...
if ~isempty(inputFile)
    ...
    if isempty(fusionFile)
        ...
        outputDir = ica_fuse_selectEntry('typeEntity', 'directory', 'title', ...
            'Select output directory for joint ICA analysis');
    else
...
figureTag = 'Setup Fusion For Joint ICA Analysis';
...
% PC2
% PC2H = findobj(handles, 'tag', 'numPC2');
%
% compStr = get(PC2H, 'string');
%
% if ~isfield(fusionInfo.setup_analysis, 'cca_opts')
%     if ~isempty(compStr)
%         numPC2 = str2num(compStr);
%     else
%         numPC2 = minPC;
%     end
% else
%     numPC2 = fusionInfo.setup_analysis.cca_opts.numPC2;
% end
%
% set(PC2H, 'enable', 'on');
% set(PC2H, 'string', num2str(numPC2));


% fusionInfo.setup_analysis.cca_opts.numPC1 = numPC1;
% fusionInfo.setup_analysis.cca_opts.numPC2 = numPC2;
...
if isfield(fusionInfo.setup_analysis, typePCATag)
    typePCAH = findobj(handles, 'tag', typePCATag);
    pcaStr = getfield(fusionInfo.setup_analysis, typePCATag);
    inds = strmatch(lower(pcaStr), char('cca', 'mcca', 'mccar'), 'exact');
    if (isempty(inds))
        inds = 1;
    end
    set(typePCAH, 'value', inds);
end
...
try
    if (strcmpi(pcaStr, 'mccar'))
        set(PC1H, 'enable', 'inactive');
    end
catch
end
...
try
    ...
    if ~isempty(inputFile)
        ...
        try
            ...
            typePCA = getfield(inputData, keywd);
            clear inputData;
        catch
        end
    end
    ...
    if (numFeatures == 1)
        error('Cannot use CCA or MCCA if the no. of features is 1');
    end
    
    if ((numFeatures > 2) && strcmpi(typePCA, 'cca'))
        error('Cannot use CCA when the no. of features is greater than 2. Please select MCCA');
    end
    ...
    % Get the number of PC1
    PC1Handle = findobj(handles, 'tag', 'numPC');
    
    numPC = str2num(get(PC1Handle, 'string'));
    
    
    if (length(numPC) == 1)
        numPC = ones(1, numFeatures)*numPC(1);
    end
    
    if (strcmpi(typePCA, 'mcca') && any(numPC ~= numPC(1)))
        warning('MCCA requires same number of principal components be entered across features. Resetting components to be equal across features');
        numPC = ones(1, numFeatures)*numPC(1);
    end
    
    if (length(numPC) ~= numFeatures)
        error('Length of principal components must match the no. of features');
    end
    
    fusionInfo.setup_analysis.cca_opts.numPC = numPC;
    ...
    %     if ~isfield(fusionInfo.setup_analysis, 'cca_opts')
    %         fusionInfo.setup_analysis.cca_opts.numPC1 = min(numSubjects);
    %         fusionInfo.setup_analysis.cca_opts.numPC2 = min(numSubjects);
    %     end
    
    newNumComp = min([numComp, numPC]);
...
switch (inputTag)
    ...
    case 'numPC'
        ...
        D(1).string = 'Skip this box for mccar. Enter number of principal components to be extracted from the data for all the features in a row vector like 5, 7';
    ...
    case 'type_pca'
        ...
        titleStr = 'Type of data reduction';
        D(1).string = 'There are two options like ''CCA'', ''MCCA'' and ''MCCAR''.';
        D(length(D) + 1).string = '';
        D(length(D) + 1).string = '1. CCA - Canonical correlation analysis is used to do data reduction. This is same as CCA + joint ICA.';
        D(length(D) + 1).string = '2. MCCA - Multimodality canonical correlation analysis is used to do data reduction. This is same as MCCA + joint ICA.';
    ...
    case 'algorithm'
        ...
        D(1).string = 'Currently, 14 ICA algorithms are available in the toolbox like Infomax, Fast ICA, Erica, Simbec, Evd, Jade Opac, Amuse, SDD ICA, CCICA, Combi, EBM, ERBM, IVA-G and IVA-GGD.';
