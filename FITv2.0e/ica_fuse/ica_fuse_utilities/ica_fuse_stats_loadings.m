function ica_fuse_stats_loadings(fileName)
...
if (~exist('fileName', 'var'))
    fileName = ica_fuse_selectEntry('title', 'Select ICA loading coefficents ...', 'typeEntity', 'file', 'typeSelection', 'single', 'filter', ...
        '*load*coeff*img;*load*coeff*nii;*mixing*coeff*nii;*mixing*coeff*img');
end
...
InputHandle = ica_fuse_getGraphics('SBM Stats GUI', 'normal', figureTag, 'on');
...
% Plot Text
designTextH = ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'text', ...
    'position', popupTextPos, 'String', 'Select design criteria', 'fontsize', UI_FONT_SIZE - 1, ...
    'horizontalalignment', 'center');

designTextH = ica_fuse_wrapStaticText(designTextH);
...
% Plot popup
designPopupH = ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'popup', ...
    'position', popupPos, 'String', desCriteriaOptions, 'fontsize', UI_FONT_SIZE - 1, 'tag', 'design_criteria', 'callback', {@designCallback, InputHandle});
...
textH = ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Add Covariates', 'tag', ...
    'prompt_covariates', 'fontsize', UI_FONT_SIZE - 1);
...
covNames = '';
ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'listbox', 'position', listboxPos, 'string', covNames, 'tag', ...
    'cov', 'fontsize', UI_FONT_SIZE - 1, 'min', 0, 'max', 2, 'value', 1,  'callback', {@addCov, InputHandle});
...
ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', addButtonPos, 'string', '+', 'tag', 'add_cov_button', 'fontsize',...
    UI_FONT_SIZE - 1, 'callback', {@addCov, InputHandle});
ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', removeButtonPos, 'string', '-', 'tag', 'remove_cov_button', 'fontsize',...
    UI_FONT_SIZE - 1, 'callback', {@removeCov, InputHandle});
...
ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', calculatePos, 'string', 'Calculate', ...
    'tag', 'calculate_button', 'fontsize', UI_FONT_SIZE - 1, 'callback', {@calculateCallback, InputHandle});
...
covFigHandle = ica_fuse_getGraphics('Select Covariates', 'normal', figureTag);
...
textH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Enter Covariate Name', 'fontsize', UI_FONT_SIZE - 1);
ica_fuse_wrapStaticText(textH);
...
editH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', covName, 'tag', 'cov_name', 'fontsize', UI_FONT_SIZE - 1);

%% Type of covariate (Continuous or categorical)
promptPos = [xOffset, editPos(2) - 0.5*promptHeight - yOffset, promptWidth, promptHeight];
textH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Select Type Of Covariate', 'fontsize', UI_FONT_SIZE - 1);
ica_fuse_wrapStaticText(textH);
...
editPos(1) = editPos(1) + editPos(3) + xOffset;
editPos(3) = controlWidth;
covH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'popup', 'position', editPos, 'string', {'Continuous', 'Categorical'}, 'tag', 'cov_type', 'fontsize', UI_FONT_SIZE - 1, ...
    'value', covTypeVal);
...
textH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Add Covariate', 'fontsize', UI_FONT_SIZE - 1);
ica_fuse_wrapStaticText(textH);
promptPos = get(textH, 'position');
...
editH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', covVals, 'fontsize', UI_FONT_SIZE - 1, 'tag', 'cov_value', 'min', 0, 'max', 2, ...
...
promptPos(1) = xOffset;
editPos = promptPos;
textH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Enter transformation function (like log, atanh). Leave it as empty if you don''t want to apply transformation.', 'fontsize', UI_FONT_SIZE - 1, ...
...
editPos(3) = controlWidth;
transformH = ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', transformationName, 'tag', 'cov_transformation', 'fontsize', UI_FONT_SIZE - 1);

okPos = [0.5 - 0.5*okWidth, yOffset + 0.5*okHeight, okWidth, okHeight];
ica_fuse_uicontrol('parent', covFigHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', okPos, 'string', 'Done', 'tag', 'cov_done', 'fontsize', UI_FONT_SIZE - 1, 'callback', ...
...
txtFile = ica_fuse_selectEntry('title', 'Select covariate file' , 'filter', '*.txt;*.asc', 'typeEntity', 'file', 'typeSelection', 'single');
drawnow;
...
try
    val = ica_fuse_load_covariates(txtFile, opts{covVal}, statsInfo.numOfSub);
    covValueH = findobj(handles, 'tag', 'cov_value');
    set(covValueH, 'string', val);
catch
    ica_fuse_errorDialog(lasterr, 'Covariate Selection');
end
...
if (~isempty(val))
    check = ica_fuse_questionDialog('title', 'Remove Covariate', 'textbody', 'Do you want to remove the covariate from the list?');
    if (~check)
        return;
    end
end
...
if (strcmpi(extn, '.nii') || strcmpi(extn, '.img'))
    loading_coeff = ica_fuse_loadData(fileName);
else
    loading_coeff = ica_fuse_load_ascii_or_mat(fileName);
end
...
try
    ...
    if (strcmpi(designCriteria, 'anova'))
        if (length(covVal) > 1)
            chkModel = ica_fuse_questionDialog('title', 'Model Interactions', 'textbody', 'Do You Want To Model Interactions?');
            if (chkModel)
    ...
    if (strcmpi(designCriteria, 'one sample t-test'))
        ...
        for nComp = 1:size(data, 2)
            tmpDat = data(:, nComp);
            tmpDat(isnan(tmpDat)) = [];
            [pValue(nComp), tValue(nComp)] = ica_fuse_ttest(tmpDat);
            tbl(nComp + 1, :) = {num2str(nComp), num2str(pValue(nComp)), num2str(tValue(nComp))};
        end
        
        ica_fuse_print_table(tbl, fid, 'a+', 0);
        fprintf(fid, '\n\n');
        fclose(fid);
    ...
    elseif (strcmpi(designCriteria, 'anova'))
        ...
        for nComp = 1:length(componentNum)
            ...
            %% Anova
            anovaR = ica_fuse_anova(tmpDat, anovaP, 'model', modelType, 'var_names', covNames);
            tbl = ica_fuse_anova_table(anovaR);
        ...
        for nComp = 1:length(componentNum)
            ...
            [rSquare_stat, beta_weights, ModelIndices, otherIndices, linearRegress, removeTrend, tmpData, ...
                subject_partial_corr] = ica_fuse_multipleRegression(tmpModelX, tmpDat, size(tmpModelX, 2), 1, ...
                length(tmpDat), 0);
    ...
    ica_fuse_errorDialog(lasterr, 'SBM Stats');
