function [groupName, groupVal] = ica_fuse_select_groups_gui(subjectString, titleString, figTag, groupName, groupVal)
...
% Plot Figure
[graphicsHandle] = ica_fuse_getGraphics(figTitle, 'normal', figTag, 'off');
...
editTextH = ica_fuse_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'text', ...
    'position', editTextPos, 'String', ['Enter name for ', titleString], 'fontsize', UI_FONT_SIZE - 1, ...
    'horizontalalignment', 'center');

[editTextH] = ica_fuse_wrapStaticText(editTextH);
...
editH = ica_fuse_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'edit', ...
    'position', editPos, 'String', groupName, 'fontsize', UI_FONT_SIZE - 1, 'tag', 'group_name');
...
listTextH = ica_fuse_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'text', ...
    'position', listTextPos, 'String', 'Select data sets', 'fontsize', UI_FONT_SIZE - 1);


[listTextH] = ica_fuse_wrapStaticText(listTextH);
...
listH = ica_fuse_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'listbox', ...
    'position', listPos, 'String', subjectString, 'fontsize', UI_FONT_SIZE - 1, 'tag', 'subject_listbox', 'min', 0, ...
...
% Plot edit control
editH = ica_fuse_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'edit', ...
    'position', editPos, 'String', num2str(get(listH, 'value')), 'fontsize', UI_FONT_SIZE - 1, 'tag', ...
    'subject_editbox');
...
okH = ica_fuse_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'pushbutton', ...
    'position', okPos, 'String', 'OK', 'fontsize', UI_FONT_SIZE - 1, 'tag', 'ok_select_subjects', 'callback', ...
...
try
    ...
    disp(lasterr);
    ica_fuse_errorDialog(lasterr, 'Error Found');
end
