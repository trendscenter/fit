function [outputText, filterAnswer] = ica_fuse_selectEntry(varargin)
...
%% Draw figure
handleFig = ica_fuse_getGraphics(figName, 'normal', 'interactiveFileWindow', 'off');
...
browseTextH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'edit', 'position', ...
...
[previousDir] = ica_fuse_showcwdHistory;

prevDirH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'popup', 'position', ...
...
% Back button
backH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', backPos, ...
    'string', 'Back', 'tooltipstring', 'Return to previous folder in the same drive...', 'callback', ...
    {@backbutton_Callback, browseTextH, handleFig});

if strcmpi(typeEntity, 'file')
    filterH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', ...
    ...
    editH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'edit', 'position', editPos, ...
    ...
    fileNoH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'edit', 'position', ...
...
driveStr = ica_fuse_getdrives;

promptString = 'Drives';

driveH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'popup', 'position', drivePos, ...
...
subfolderH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'listbox', 'position', ...
...
entryH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'listbox', 'position', entryPos, 'tag', tag, ...
    'BackgroundColor', UI_BG_COLOR, 'horizontalalignment', 'left', 'userdata', pwd, 'callback', {@filePrefixListbox, handleFig});

% Create a push button here
okH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', okPos, 'string', 'OK', 'BackgroundColor', BUTTON_BG_COLOR,...
    'tooltipstring', 'Select Entries...', 'callback', {@doOK, handleFig}, 'tag', 'doOk');

% Create a push button here
refreshH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', ...
...
if strcmpi(typeEntity, 'file')
    ...
    % Create a push button here
    buttonH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', buttonPos, ...
...
frametH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'frame', 'position', framePos);
% Show the user number of files selected
staticTextH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'text', 'position', fileInfoTextPos, 'BackgroundColor', UI_BG_COLOR,...
    'ForegroundColor', UI_FG_COLOR, 'fontunits', UI_FONT_UNITS, 'fontname', UI_FONT_NAME, 'FontSize', UI_FONT_SIZE - 1, 'string', stringStaticText, ...
...
if strcmpi(typeSelection, 'multiple') && strcmpi(typeEntity, 'file')
    ...
    % Create a push button here
    select_all_H = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', select_all_pos, 'string', 'Select-all', ...
        'BackgroundColor', BUTTON_BG_COLOR, 'ForegroundColor', BUTTON_FG_COLOR, 'fontunits', UI_FONT_UNITS, 'fontname', UI_FONT_NAME, 'FontSize', UI_FONT_SIZE, ...
    ...
    editTextH = ica_fuse_uicontrol('parent', handleFig, 'units', 'normalized', 'style', 'pushbutton', 'position', ...
...
try
    ...
    if check_files == 0
        ...
        dirContents = ica_fuse_listFiles_inDir(pwd, stringVar);
        if strcmpi(fileType, 'image')
            dirContents = ica_fuse_rename_4d_file(dirContents, fileNumbers);
        end
        drawnow;
        ...
        if ~isempty(dirContents)
            ...
            if strcmpi(getUserData{1}, 'prefix')
                [summaryContents, indices_prefix] = ica_fuse_classifyFiles(dirContents);
...
currentChar = ica_fuse_fullFile('directory', currentDir, 'files', deblank(getString(selection(length(selection)), :)));
...
if ~strcmpi(get(handles, 'selectionType'), 'open')
    ...
    if check_files == 0
        ...
        if length(getValue) == size(getString, 1)
            ...
            fullFileContents = ica_fuse_fullFile('directory', pwd, 'files', fileContents); % full file contents
            ...
            if strcmpi(getUserData{1}, 'prefix')
                ...
                for ii = 1:length(getValue)
                    ...
                    fullFileContents = ica_fuse_fullFile('directory', pwd, 'files', fileContents);
                    if ~isempty(fullFileContents)
                        % store the files
                        if ~isempty(temp.files)
                            temp.files = str2mat(temp.files, fullFileContents);
                ...
                % List files
                fileContents = ica_fuse_fullFile('directory', pwd, 'files', allFileNames(getValue, :));
                % check the select mode
                if get(handleObj, 'max') == 1
                    set(handleObj, 'enable', 'inactive');
                    tempVariable = deblank(fileContents(1, :));
        ...
        [newDirs, uniqueInd] = ica_fuse_uniqueStr(cellstr(str2mat(dirInfo.directories)), 'dir');
...
if check_folder == 0
    ...
    if ~isempty(okData.directory)
        ...
        ica_fuse_errorDialog('Directory is not selected. Please select the directory in the right listbox.', ...
...
try
    ...
    newFigureHandle = ica_fuse_getGraphics('Edit Files ...', 'normal', 'edit_files_figure');
    ...
    editTextH = ica_fuse_uicontrol('parent', newFigureHandle, 'units', 'normalized', 'style', 'edit', 'position', ...
    ...
    cancelH = ica_fuse_uicontrol('parent', newFigureHandle, 'units', 'normalized', 'style', 'pushbutton', ...
    ...
    okH = ica_fuse_uicontrol('parent', newFigureHandle, 'units', 'normalized', 'style', 'pushbutton', ...
...
try
    ...
    [answerQuestion] = ica_fuse_questionDialog('title', 'Number of files', 'textbody', msgString);
