function varargout = ica_fuse_ui_select_data(varargin)
...
for ii = 1:2:nargin
    ...
    elseif strcmpi(varargin{ii}, 'num_sessions')
        num_sessions = varargin{ii + 1};
    elseif strcmpi(varargin{ii}, 'files_specification')
        files_specification = lower(varargin{ii + 1});
    elseif strcmpi(varargin{ii}, 'type_file_selection')
        type_file_selection = lower(varargin{ii + 1});
    elseif strcmpi(varargin{ii}, 'spm_check')
        spm_check = lower(varargin{ii + 1});
    elseif strcmpi(varargin{ii}, 'subject_string')
        subject_string = varargin{ii + 1};
    elseif strcmpi(varargin{ii}, 'filter_string')
    ...
    elseif strcmpi(varargin{ii}, 'fileinfo')
        fileInfo = varargin{ii + 1};
    elseif strcmpi(varargin{ii}, 'figure_menu')
...
if (~iscell(fileType))
    fileType = repmat({fileType}, 1, num_subjects*num_sessions);
end
...
figureData = struct('oldDir', pwd, 'pwd', startPath, 'num_subjects', num_subjects, 'num_sessions', num_sessions, ...
    ...
    'changeFiles', 'no', 'dataType', dataType, 'filter_answer', filter_answer, ...
    'file_ext', fileExtension, 'fileNumbers', fileNumbers);
...
%     'BackgroundColor', BUTTON_BG_COLOR, 'ForegroundColor', BUTTON_FONT_COLOR, 'fontunits', UI_FONTUNITS, 'fontname', ...
%     UI_FONTNAME, 'FontSize', UI_FONT_SIZE, 'string', 'Save', 'tag', 'save', 'callback', {@saveCallback, listH}, 'userdata', fileInfo);
...
try
    ...
    fileTypes = figureData.fileType;
    file_ext = figureData.file_ext;
    fileNumbers = figureData.fileNumbers;
    
    fileType = deblank(fileTypes{getValue});
    ...
    if isempty(dataFiles(getValue).name) || ~strcmp(lower(figureData.changeFiles), 'no')
        ...
        [dataFiles(getValue).name, filter_answer] = ica_fuse_selectEntry('typeEntity', 'file', 'typeselection', type_file_selection, ...
            'num_datasets', 2, 'count_datasets', 1, 'filter', filter_answer, 'title', promptString, 'startPath', ...
            tempDir, 'fileNumbers', fileNumbers, 'fileType', fileType);
    end
    ...
    if strcmpi(fileType, 'image')
        ...
        [nn, mm, dims] = ica_fuse_get_countTimePoints(ica_fuse_parseExtn(checkFiles{1}));
        ...
        if strcmp(dataType, 'complex')
            ...
            for nF = 1:length(fileNames)
                ...
                checkExtn = ica_fuse_findstr(chk, '.');
            ...
            if length(good_inds) ~= length(fileNames)
                error('Please check the READ_NAMING_COMPLEX_IMAGES variable in ica_fuse_defaults.m.');
            end
        end
    ...
    set(handles, 'pointer', 'arrow');
    ica_fuse_errorDialog(lasterr, 'File selection error');
    disp(lasterr);
end
...
try
    ...
    for ii = 1:length(dataFiles)
        ...
        if (strcmpi(files_specification, 'equal'))
            if ii > 1
        ...
        elseif (strcmpi(files_specification, 'equal_sessions'))
            % Handle the case where time points are equal between sessions
            
            % Subject number
            subNumber = ceil(ii/numOfSess);
            % Session number
            sessNumber = ii - ((subNumber - 1)*numOfSess);
            
            if (sessNumber > 1)
                grp1Num = ((subNumber - 1)*numOfSess) + 1;
                grp1Tp = numFiles(grp1Num);
                if (grp1Tp ~= numFiles(ii))
                    error(['No. of files selected for ', deblank(listString(ii, :)), ' must be equal to the no. of files selected for ', ...
                        deblank(listString(grp1Num, :))]);
                end
            end
...
if strcmp(lower(fileFormat), 'append')
    ...
    ica_fuse_save(fileName, 'SPMFiles', '-append');
else
    files = get(handles, 'userdata');
    SPMFiles.name = [];
    ica_fuse_save(fileName, 'files', 'SPMFiles', 'numOfSub', 'numOfSess');
end
...
disp(dispString);
% open dialog box
figHandle = ica_fuse_dialogBox('title', 'File Saved', 'textBody', dispString, 'textType', 'large');
waitfor(figHandle);
...
dialogHandle = ica_fuse_dialogBox('title', 'Information on number of images for each dataset', 'textBody', ...
...
good_inds = ica_fuse_good_cells(checkExtn);
