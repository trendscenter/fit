function str = ica_fuse_spm_file(str,varargin)
...
if numel(options) == 1
    for n=1:numel(str)
        [pth,nam,ext,num] = ica_fuse_spm_fileparts(deblank(str{n}));
        switch lower(options{1})
            ...
            case 'number'
                str{n} = num;
            case 'cpath'
                str{n} = ica_fuse_spm_select('CPath',str{n});
            case 'fpath'
                str{n} = ica_fuse_spm_fileparts(ica_fuse_spm_select('CPath',str{n}));
            case 'unique'
                i = 1;
                while true
                    str{n} = fullfile(pth,sprintf('%s_%03d%s',nam,i,ext));
                    if ~ica_fuse_spm_existfile(str{n}), break; else i = i + 1; end
                end
                str{n} = [str{n} num];
            case 'uniquedir'
                i = 1;
...
while ~isempty(options)
    for n=1:numel(str)
        ...
        switch lower(options{1})
            ...
            case 'link'
                if ica_fuse_spm_platform('desktop')
