function x = ica_fuse_spm_coreg(varargin)
...
%def_flags          = ica_fuse_spm_get_defaults('coreg.estimate');
...
if nargin < 1
    VG = ica_fuse_spm_vol(ica_fuse_spm_select(1,'image','Select reference image'));
else
    VG = varargin{1};
    if ischar(VG), VG = ica_fuse_spm_vol(VG); end
end
if nargin < 2
    VF = ica_fuse_spm_vol(ica_fuse_spm_select(Inf,'image','Select moved image(s)'));
else
    VF = varargin{2};
    if ischar(VF) || iscellstr(VF), VF = ica_fuse_spm_vol(char(VF)); end;
end
...
for k=1:numel(VF)
    ...
    xk  = flags.params(:);
    for samp=flags.sep(:)'
        xk     = ica_fuse_spm_powell(xk(:), xi,sc,mfilename,VG,VFk,samp,flags.cost_fun,flags.fwhm);
        x(k,:) = xk(:)';
    end
    if flags.graphics
...
H = ica_fuse_spm_hist2(VG.uint8,VF.uint8, VF.mat\ica_fuse_spm_matrix(x(:)')*VG.mat ,sg);

% Smooth the histogram
lim  = ceil(2*fwhm);
krn1 = ica_fuse_spm_smoothkern(fwhm(1),-lim(1):lim(1)) ; krn1 = krn1/sum(krn1); H = conv2(H,krn1);
krn2 = ica_fuse_spm_smoothkern(fwhm(2),-lim(2):lim(2))'; krn2 = krn2/sum(krn2); H = conv2(H,krn2);
...
if size(V.pinfo,2)==1 && V.pinfo(1) == 2
    ...
    %     ica_fuse_spm_progress_bar('Init',V.dim(3),...
    %         ['Computing max/min of ' ica_fuse_spm_file(V.fname,'filename')],...
    %         'Planes complete');
    mx = -Inf; mn =  Inf;
    for p=1:V.dim(3)
        img = ica_fuse_spm_slice_vol(V,ica_fuse_spm_matrix([0 0 p]),V.dim(1:2),1);
        img = img(isfinite(img));
        mx  = max([max(img(:))+paccuracy(V,p) mx]);
        mn  = min([min(img(:)) mn]);
        %  ica_fuse_spm_progress_bar('Set',p);
    end
end

% Another pass to find a maximum that allows a few hot-spots in the data.
% ica_fuse_spm_progress_bar('Init',V.dim(3),...
%         ['2nd pass max/min of ' ica_fuse_spm_file(V.fname,'filename')],...
%         'Planes complete');
nh = 2048;
h  = zeros(nh,1);
for p=1:V.dim(3)
...
% % Load data from file indicated by V into an array of unsigned bytes.
% ica_fuse_spm_progress_bar('Init',V.dim(3),...
%     ['Loading ' ica_fuse_spm_file(V.fname,'filename')],...
%     'Planes loaded');
...
%ica_fuse_spm_progress_bar('Clear');
...
%==========================================================================
function acc = paccuracy(V,p)
if ~ica_fuse_spm_type(V.dt(1),'intt')
    acc = 0;
else
    if size(V.pinfo,2)==1
        acc = abs(V.pinfo(1,1));
...
lim = ceil(2*fwhm);
x  = -lim(1):lim(1); x = ica_fuse_spm_smoothkern(fwhm(1),x); x  = x/sum(x);
y  = -lim(2):lim(2); y = ica_fuse_spm_smoothkern(fwhm(2),y); y  = y/sum(y);
z  = -lim(3):lim(3); z = ica_fuse_spm_smoothkern(fwhm(3),z); z  = z/sum(z);
i  = (length(x) - 1)/2;
j  = (length(y) - 1)/2;
k  = (length(z) - 1)/2;
ica_fuse_spm_conv_vol(V.uint8,V.uint8,x,y,z,-[i j k]);
...
%==========================================================================
function display_results(VG,VF,x,flags)
fig = ica_fuse_spm_figure('FindWin','Graphics');
if isempty(fig), return; end;
set(0,'CurrentFigure',fig);
ica_fuse_spm_figure('Clear','Graphics');
...
Q = inv(VF.mat\ica_fuse_spm_matrix(x(:)')*VG.mat);
...
H   = ica_fuse_spm_hist2(VG.uint8,VF.uint8,VF.mat\VG.mat,[1 1 1]);
tmp = log(H+1);
...
xlabel(ica_fuse_spm_file(VG.fname,'short22'),'Parent',ax,'Interpreter','none');
ylabel(ica_fuse_spm_file(VF.fname,'short22'),'Parent',ax,'Interpreter','none');

H   = ica_fuse_spm_hist2(VG.uint8,VF.uint8,VF.mat\ica_fuse_spm_matrix(x(:)')*VG.mat,[1 1 1]);
...
% Display ortho-views
%--------------------------------------------------------------------------
ica_fuse_spm_orthviews('Reset');
ica_fuse_spm_orthviews('Image',VG,[0.01 0.01 .48 .49]);
h2 = ica_fuse_spm_orthviews('Image',VF,[0.51 0.01 .48 .49]);
global st
st.vols{h2}.premul = inv(ica_fuse_spm_matrix(x(:)'));
ica_fuse_spm_orthviews('Space');
