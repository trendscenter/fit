function V = ica_fuse_spm_create_vol(V,varargin)
% Create a volume
...
if ~isfield(V,'dt'),
    V.dt = [ica_fuse_spm_type('float64') ica_fuse_spm_platform('bigend')];
end;

dt{1} = ica_fuse_spm_type(V.dt(1));
...
bits   = ica_fuse_spm_type(V.dt(1),'bits');
...
dim    = [V.dim(1:3) V.n];
dat    = ica_fuse_file_array(V.fname,dim,[dt{1} '-' dt{2}],0,V.pinfo(1),V.pinfo(2));
N      = ica_fuse_nifti;
N.dat  = dat;
N.mat  = V.mat;
N.mat0 = V.mat;
