function [Y] = ica_fuse_spm_get_data(V,XYZ,check)
...
for i = 1:length(V)
    ...
    %----------------------------------------------------------------------
    if check && ~ica_fuse_spm_existfile(V(i).fname)
    ...
    %-Load data
    %----------------------------------------------------------------------
    Y(i,:) = ica_fuse_spm_sample_vol(V(i),XYZ(1,:),XYZ(2,:),XYZ(3,:),0);
