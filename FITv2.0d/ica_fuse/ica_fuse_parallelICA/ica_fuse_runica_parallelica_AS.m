% runica _parallel() - Perform parallel Independent Component Analysis (ICA) decomposition
%            of psychophysiological data , with correlation contrains of an  A matrix and a source
%            singale, using the infomax ICA algorithm of
...
% Usage:
%      simply >> [weights,sphere] = runica_parallel(data,'dewhitem',{dewhiteMfMRI},'whitem',{whiteMfMRI},...
%                           'constrained_components',2,'constrained_connection',0.30 );
%       or
...
% Key   Keywods value
% 'dewhitem'    = cell type Dewhite matrix {dewhiteM1}.. 
% 'whitem'      = cell type white matrix {whiteM1}.. 
...
function [weights,sphere,activations,bias,signs,lrates,y] = ica_fuse_runica_parallelica_AS(data,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6,p7,v7,p8,v8,p9,v9,p10,v10,p11,v11,p12,v12,p13,v13,p14,v14)

if nargin < 5
    help runica_parallel_new
    return
end

% separte two datasets
data1=data{1};
data2=data{2};
clear data;
...
DEFAULT_MAXSTEPS     = 1000;       % ]top training after this many steps 512
...
DEFAULT_LRATE        = 0.015./log(chans);% chnaged by jy; original 0.015./log(chans)
...
while step(1) < maxsteps | step(2) < maxsteps
    ...
    % ------------------------- 
    % modifying weights based on correlation between data1 A Matrix and data2 signal w*x 
    % %%%%%%%%%%%%%%%%%nudging
    ...
    if (step(1)>2 & step(2)>2) & ( ~stopsign(1) | ~stopsign(2))
       ...
       % for data2, calculalte singles from w*data2
       sx = weights{2}*data2;
           ...
           for Cons_com=index(1:length(ix)); % constraned componenets
             ...
             %    [maxcorr,tmcorr,maxrow ,maxcol]
               if ~stopsign(1)
                    ...
                    weights_temp= whiteM{1}*mx;
                    weights_temp=inv(weights_temp);% weights{1} \ eye(size(weights{1}));
                    weights{1}=weights_temp;
               ...
               if ~stopsign(2)
                    deltaf=(comterm*( a*data2'-coml*mean(a)*mean(data2') + ...
                                              b/var(u)*(coml*mean(u)*mean(data2') -u*data2') ));% 1st order derivative
                    P=deltaf./norm(deltaf) ;% (H2*deltaf')';                          
                    alphak(2)=findsteplength2(-tmcorr^2,-deltaf,weights{2}(maxrow(Cons_com),:),a,alphak(2),P,0.0001,0.999,data2);
...
% figure;plot(lossf1);
% figure;plot(lossf2);
% if exist('lossf3') figure;plot(lossf3);end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%plot the  correlated components
% figure; plot( a,'r.');hold on;plot(u,'b.'); title('Correlated componenets')
% legend('fMRI','SNPs')
...
function alphak=findsteplength2(fk,deltafk,x1,x2,alphak,P,c1,c2,xdata);
% 0<c1<0.5<c2<1;
% (f(x+ap)-f(x))/c1/dela_f(x)/P <a
% x1 is weight for u=wx; x2 is a.
    con=1;coml=length(x2);
    while con & con<100
        xnew=(x1+alphak*P)*xdata; % u
        fk1=corrcoef(xnew,x2);fk1=-fk1(2)^2;
        tcov=((xnew-mean(xnew))*(x2-mean(x2))')/(coml-1);
        comterm=2*tcov/cov(xnew)/cov(x2); 
        deltafk1=-(comterm*( x2*xdata'-coml*mean(x2)*mean(xdata') + ...
                                       tcov/var(xnew)*(coml*mean(xnew)*mean(xdata') -xnew*xdata') ));% 1st order derivative
        firstterm1=(fk1-fk)/c1;firstterm2=deltafk*P(:);
        secondterm1=deltafk1*P(:);secondterm2=deltafk*P(:);
    ...
    if con>=50 alphak=0;disp('learning rate searching for SNP data failed!'); end