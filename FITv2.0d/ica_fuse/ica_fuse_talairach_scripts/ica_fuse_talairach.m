function ica_fuse_talairach(files, threshold, convertToZ, sample_size)
...
if ~exist('files', 'var')
    isGUI = 1;
    files = ica_fuse_selectEntry('typeSelection', 'multiple', 'typeEntity', 'file', 'fileType', 'image', 'title', 'Select image/images ...', 'filter', ...
        '*.img;*.nii');
end
...
for nFiles = 1:length(files)
    ...
    for nV = 1:length(V)
        ...
        if ((length(V) > 1) || (V(nV).n(1) > 1))
            file_name = [file_name, '_', ica_fuse_returnFileIndex(V(nV).n(1))];
        end
        
        data = ica_fuse_spm_read_vols(V(nV));
        data(isnan(data)) = 0;
