function varargout = jointICA_fusion(varargin)
% JOINTICA_FUSION M-file for jointICA_fusion.fig
%      JOINTICA_FUSION, by itself, creates a new JOINTICA_FUSION or raises the existing
%      singleton*.
%
%      H = JOINTICA_FUSION returns the handle to a new JOINTICA_FUSION or the handle to
%      the existing singleton*.
%
%      JOINTICA_FUSION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in JOINTICA_FUSION.M with the given input arguments.
%
%      JOINTICA_FUSION('Property','Value',...) creates a new JOINTICA_FUSION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before jointICA_fusion_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to jointICA_fusion_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help jointICA_fusion

% Last Modified by GUIDE v2.5 07-Mar-2017 23:53:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @jointICA_fusion_OpeningFcn, ...
    'gui_OutputFcn',  @jointICA_fusion_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before jointICA_fusion is made visible.
function jointICA_fusion_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to jointICA_fusion (see VARARGIN)

% Choose default command line output for jointICA_fusion
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes jointICA_fusion wait for user response (see UIRESUME)
...
function varargout = jointICA_fusion_OutputFcn(hObject, eventdata, handles)
