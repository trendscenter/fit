function [images, coords, HInfo, slices, text_left_right] = ica_fuse_resizeImage(structVol, compFile, anatomicalView, slices, file_numbers, spmPath)
...
if ~exist('file_numbers', 'var')
    error('component file number is missing');
end
...
% slice definition
[parameters] = ica_fuse_get_slice_def(structVol, anatomicalView);
...
extn_comp = ica_fuse_parseExtn(extn_comp);

% Initialise img structure
img = repmat(struct('V', []), numComp + 1, 1);

% check the image extension
if strcmpi(extn_comp, '.img')
    % analyze convention
    ...
    for ii = 1:length(img)
        if ii == 1
            ...
            img(ii).V = ica_fuse_spm_vol(deblank(compFile(file_numbers(ii - 1), :)));
...
elseif strcmpi(extn_comp, '.nii')
    ...
    for ii = 1:length(img)
        if ii == 1
            ...
            img(ii).V = ica_fuse_get_vol_nifti([compFile, ',', num2str(file_numbers(ii-1))]);
...
for ii = 1:nslices
    ...
    for jj = 1 : numComp + 1
        ...
        % raw data
        i1 = ica_fuse_spm_sample_vol(img(jj).V(1), vixyz(X, :), vixyz(Y, :), vixyz(Z, :), [1, nan]);
