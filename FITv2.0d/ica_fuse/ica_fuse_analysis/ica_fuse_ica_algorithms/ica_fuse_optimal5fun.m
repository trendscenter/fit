function [name,a,b] = ica_fuseoptimal5fun(data)
% [name,a,b] = ica_fuseoptimal5fun(data)
...
% decide which distribution is optimal  
    for i=1:length(x1)  
         y1 = ica_fuselogistic_pdf(x1(i),0,1);  %%% here mean=0; var=1;
         y2 = ica_fuselap_pdf(x1(i),meanS1,sqrt(varS1/2));
         y3 = normpdf(x1(i),meanS1,sqrt(varS1));
         y4 = ica_fuselogweibull_pdf(x1(i),a1,b1);
         ...
         min1 = min(delt1);
     for i=1:length(x2)  
         y1 = ica_fuselogistic_pdf(x2(i),0,1);  %%% here mean=0; var=1;
         y2 = ica_fuselap_pdf(x2(i),meanS2,sqrt(varS2/2));
         y3 = normpdf(x2(i),meanS2,sqrt(varS2));
         y4 = ica_fuselogweibull_pdf(x2(i),a2,b2);
