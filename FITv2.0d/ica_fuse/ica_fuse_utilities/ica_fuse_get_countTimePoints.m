function [diffTimePoints, extns, dims] = ica_fuse_get_countTimePoints(files)
...
% 
% Output:
% 1. diffTimePoints - Count for the time points as a vector
% 2. extns - Extensions structure
% 3. dims - data dimensions
...
for ii = 1:length(files)
    ...
    for jj = 1:size(currentFiles, 1)
        ...
        if strcmpi(extns.data(ii).file(jj).ext, '.nii')
            % read the time point dimensions
            [hdr] = ica_fuse_read_hdr(deblank(currentFiles(jj, :)));            
        ...
        elseif strcmpi(extns.data(ii).file(jj).ext, '.img')
            ...
            [hdr, otherendian] = ica_fuse_read_hdr(hdrFile);            
