function fusionFile = ica_fuse_setup_analysis_tiva(inputFile, fusionFile)
...
[inputText] = ica_fuse_define_parameters_tiva(inputFile);
...
if ~isempty(inputFile)
    ...
    if isempty(fusionFile)
        ...
        outputDir = ica_fuse_selectEntry('typeEntity', 'directory', 'title', ...
            'Select output directory for tIVA analysis');
    else
...
figureTag = 'Setup Fusion For tIVA Analysis';
...
%     PCAVal = get(typePCAH, 'value');
    ...
    if (strcmpi(typePCA, 'cca') || strcmpi(typePCA, 'mcca'))
        
        if ~isfield(fusionInfo.setup_analysis, 'cca_opts')
            fusionInfo.setup_analysis.cca_opts.numPC = repmat(min(numSubjects), 1, numFeatures);
        end
        ...
        if (strcmpi(typePCA, 'mcca') && any(fusionInfo.setup_analysis.cca_opts.numPC ~= fusionInfo.setup_analysis.cca_opts.numPC(1)))
            ...
            fusionInfo.setup_analysis.cca_opts.numPC = ones(1, numFeatures)*fusionInfo.setup_analysis.cca_opts.numPC(1);
        end
        
        if (length(fusionInfo.setup_analysis.cca_opts.numPC) == 1)
            fusionInfo.setup_analysis.cca_opts.numPC = ones(1, numFeatures)*fusionInfo.setup_analysis.cca_opts.numPC(1);
        end
        
        if (length(fusionInfo.setup_analysis.cca_opts.numPC) ~= numFeatures)
            error('Length of principal components must equal no. of features when you select MCCA or CCA');
        end
...
switch (inputTag)
    ...
    case 'algorithm'
        ...
        titleStr = 'ICA/IVA Algorithm';
        D(1).string = 'Currently, 14 ICA/IVA algorithms are available in the toolbox like Infomax, Fast ICA, Erica, Simbec, Evd, Jade Opac, Amuse, SDD ICA, CCICA, Combi, EBM, ERBM, IVA-G and IVA-GGD.';
