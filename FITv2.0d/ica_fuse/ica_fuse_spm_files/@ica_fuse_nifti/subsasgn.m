...
case {'.'},
    ...
    for i=1:length(varargin),
        val     = varargin{i};
        obji    = class(objs(i),'ica_fuse_nifti');
        obji    = fun(obji,subs,val);
        objs(i) = struct(obji);
    end;
    obj = class(objs,'ica_fuse_nifti');
...
case {'.'},
    ...
    for ii=1:numel(objs)
        ...
        if length(subs)>1, % && ~strcmpi(subs(1).subs,{'raw','dat'}),
            val0 = subsref(class(obj,'ica_fuse_nifti'),subs(1));
...
if isa(val,'ica_fuse_file_array'),
    sz = size(val);
    if numel(sz)>8,
    ...
    error('"raw" must be of class "ica_fuse_file_array"');
end;
return;
