function [Y,XYZ] = ica_fuse_spm_read_vols(V,mask)
% Read in entire image volumes
% FORMAT [Y,XYZ] = ica_fuse_spm_read_vols(V,mask)
% V    - vector of mapped image volumes to read in (from ica_fuse_spm_vol)
...
%
% For image data types without a representation of NaN (see ica_fuse_spm_type),
...
% Andrew Holmes
% $Id: ica_fuse_spm_read_vols.m 3663 2010-01-07 12:26:48Z guillaume $
...
ica_fuse_spm_check_orientations(V);
...
for i=1:n, for p=1:V(1).dim(3)
    Y(:,:,p,i) = ica_fuse_spm_slice_vol(V(i),ica_fuse_spm_matrix([0 0 p]),V(i).dim(1:2),0);
end, end
...
if mask
    ...
    im = false(n,1);
    for i=1:n, im(i)=~ica_fuse_spm_type(V(i).dt(1),'NaNrep'); end
    %-Mask
    Y(Y(:,:,:,im)==0) = NaN;
end
