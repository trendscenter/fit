...
if ~strcmp(subs(1).type,'()'),
    if strcmp(subs(1).type,'.'),
        ...
        if numel(struct(obj))~=1,
            error('Can only change the fields of simple ica_fuse_file_array objects.');
        end;
        switch(subs(1).subs)
...
if length(subs.subs) < length(dm),
    ...
    if numel(sobj) ~= 1,
        error('Can only reshape simple ica_fuse_file_array objects.');
    end;
    if numel(sobj.scl_slope)>1 || numel(sobj.scl_inter)>1,
        error('Can not reshape ica_fuse_file_array objects with multiple slopes and intercepts.');
    end;
end;
...
elseif nelem==2,
    ...
    ica_fuse_mat2file(sobj1,dat,int32([1 2]),va{:});
else
