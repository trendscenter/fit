function  varargout = ica_fuse_display_composite(files, varargin)
...
template_file = fullfile(fileparts(which('fusion.m')), 'ica_fuse_templates', 'ch2bet.nii');
slice_plane = 'axial';
slices_in_mm = [];
axesTitle = '';
display_type = 'ortho';
...
if (~exist('files', 'var') || isempty(files))
    files = ica_fuse_selectEntry('title', 'Select images ...', 'typeSelection', 'multiple', 'typeEntity', 'file', 'filter', '*img;*nii', 'fileType', 'image');
    drawnow;
end
...
structVol = ica_fuse_spm_vol(template_file);
...
if (~strcmpi(display_type, 'render'))
    ...
    for nComp = 1:length(files)
        ...
        [tmpF, tmpN] = ica_fuse_parseExtn(fileName);
        
        [imageTmp, coordsAll, HInfo, slices_in_mm] = ica_fuse_resizeImage(structVol, tmpF, slice_plane, slices_in_mm, tmpN);
        ...
        imageTmp = ica_fuse_applyDispParameters('image', imageTmp(:)', strcmpi(convert_to_zscores, 'yes'), returnValue, threshold);
    ...
    voxelOrigin = ica_fuse_real_to_voxel(structVol(1), handles_data.origin );
    ...
    if (strcmpi(display_type, 'montage'))
        [im, numImagesX, numImagesY, slices_in_mm] = ica_fuse_returnMontage(compositeMap, [], size(compositeMap), handles_data.VOX, slices_in_mm);
    ...
    gH = ica_fuse_getGraphics('Composite Maps', 'graphics', 'composite_maps', 'on');
    set(gH, 'resize', 'on');
    ...
    if (strcmpi(display_type, 'ortho'))
            ...
            for nTextRows = 1:numImagesY
                ...
                for nTextCols = 1:numImagesX
                    ...
                    if textCount <= structDIM(3)
                        txHandle(textCount) = text(xPos, yPos, num2str(round(slices_in_mm(textCount))), 'color', UI_FG_COLOR,  ...
                            'fontsize', UI_FONT_SIZE-4, 'HorizontalAlignment', 'left', 'verticalalignment', 'bottom', ...
                            'FontName', UI_FONT_NAME, 'parent', sh);
                    end
        ...
        %             imagesc(dd);
        %             xlabel(num2str(slices_in_mm(count)), 'parent', sh, 'horizontalAlignment', 'left', 'color', UI_FG_COLOR);
        %             axis image;
        %             %axis off;
    ...
    for nV = 1:length(files)
        ...
        [V, HInfo] = ica_fuse_returnHInfo(files{nV});
        [R, C, P] = ndgrid(1:V(1).dim(1),1:V(1).dim(2),1:V(1).dim(3));
        RCP = [R(:)';C(:)';P(:)'];
        data = ica_fuse_spm_read_vols(V);
        data(isfinite(data) == 0) = 0;
        data = ica_fuse_applyDispParameters('image', data(:)', strcmpi(convert_to_zscores, 'yes'), returnValue, threshold, V.dim(1:3), HInfo);
        ...
        [blah, rendMat{nV}] = ica_fuse_spm_render(dat, NaN, rendfile);
    ...
    axesPos = [0.15, 0.25, 0.7, 0.7];
    gH = ica_fuse_getGraphics('Composite Renderer', 'graphics', 'Image Viewer', 'on');
...
for n = 1:length(files)
    ...
    xlabel(char(minMaxLabels{n, 1}, '', labels{n}), 'parent', ch, 'fontsize', UI_FONT_SIZE-4);
    title(minMaxLabels{n, 2}, 'parent', ch, 'fontsize', UI_FONT_SIZE-4, 'color', UI_FG_COLOR);
    set(ch, 'color', UI_FG_COLOR);
    allColorbars(n) = ch;
end
...
%voxelCoord = round(ica_fuse_real_to_voxel(handles_data.structVol, realCoords));
...
%realCoords = (ica_fuse_voxel_to_real(structVol, coords(ncoords, :)));
...
xlabel(['X = ', num2str(realCoords(1)), ' mm'], 'parent', sh, 'color', UI_FG_COLOR);
...
xlabel(['Y = ', num2str(realCoords(2)), ' mm'], 'parent', sh, 'color', UI_FG_COLOR);
...
xlabel(['Z = ', num2str(realCoords(3)), ' mm'], 'parent', sh, 'color', UI_FG_COLOR);
...
for nX = 1:length(xlabelH)
    if (iscell(xlabelH))
        set(xlabelH{nX}, 'color', UI_FG_COLOR);
    else
        set(xlabelH(nX), 'color', UI_FG_COLOR);
    end
end
...
set(gH, 'userdata', handles_data);
title(handles_data.title, 'parent', allAxes(2), 'fontsize', 12, 'color', UI_FG_COLOR);
...
%def = {num2str((ica_fuse_voxel_to_real(handles_data.structVol, handles_data.coords)))};

% get voxel coords
voxelPos = ica_fuse_inputdlg2(prompt, dlg_title, num_lines, def);
drawnow;
...
if (~isempty(voxelPos))
    ...
    %handles_data.coords = round(ica_fuse_real_to_voxel(handles_data.structVol, voxelPos));
    handles_data.coords = voxelPos;
    %voxelOrigin = ica_fuse_real_to_voxel(handles_data.structVol(1), handles_data.origin);
...
%realCoords = (ica_fuse_voxel_to_real(handles_data.structVol, handles_data.coords(1, :)));
colorLen = handles_data.colorLen;
...
voxelCoord = ica_fuse_real_to_voxel(V, realWorldPos);
...
function [V, HInfo] = ica_fuse_returnHInfo(P)
...
gzipFile = 0;
if (ica_fuse_findstr(lower(P), '.gz'))
    gzipFile = 1;
end

if (~gzipFile)
    if ica_fuse_findstr(lower(P), '.gii')
        gd = gifti(P);
        V(1).fname = P;
        ...
        if (V(1).private.hdr.dim(1) == 6)
            ...
            dat = ft_read_cifti(ica_fuse_parseExtn(P));
            dat.dtseries = [];
            V(1).cifti.dat=dat;
        end
    ...
    [hdr, V] = ica_fuse_read_gzip_nii(ica_fuse_parseExtn(P), 'read_hdr_only', 1);
    VOX = hdr.pixdim(2:4);
end
