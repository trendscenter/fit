function varargout = ica_fuse_orth_views(file, varargin)
%% Orthogonal viewer
%
% Inputs:
% 1. file - Input file
...
[structFile, n] = ica_fuse_parseExtn(structFile);
structVol = ica_fuse_spm_vol_nifti(structFile, n);
structVol = structVol(1);

[images, real_world_coords] = ica_fuse_resizeImage(structVol, file, 'axial', [], 1);
...
if (~set_to_max_voxel)
    pixelPos = round(ica_fuse_real_to_voxel(structVol, coords(1, :)));
else
    pixelPos = [x, y, z];
end
...
if (isempty(cmap))
    cmap = ica_fuse_getColormap(returnValue, 1);
    if (returnValue == 1)
...
%coords(nC, :) = round(ica_fuse_real_to_voxel(structVol, coords(nC, :)));
%end
...
if (returnInterpdata)
    ...
    handles_data.dispPrefs = struct('returnValue', returnValue, 'stdData', stdData, 'meanData', meanData, 'imgVol', ...
        ica_fuse_returnHInfo(file), 'threshold', threshold);
end
...
handles_data.voxelOrigin = ica_fuse_real_to_voxel(structVol(1), handles_data.origin);
...
%% Figure and Menus
gH = ica_fuse_getGraphics(fig_title, 'graphics', 'orthoviews', 'off');
...
if (exist('tc', 'var'))
    ...
    ica_fuse_plot_spectra(axesH, tc);
end

axisH = axes('Parent', gH, 'position', [0 0 1 1], 'visible', 'off');
xPos = 0.5; yPos = 0.97;
titleColor = 'c';
text(xPos, yPos, fig_title, 'color', titleColor, 'fontweight', 'bold', 'fontsize', UI_FONT_SIZE + 1, 'HorizontalAlignment', 'center', 'FontName', UI_FONT_NAME, 'parent', axisH);
...
parameters = ica_fuse_get_slice_def(structVol, 'axial');
...
for ii = 1:nslices
    ...
    i1 = ica_fuse_spm_sample_vol(imgVol(1), vixyz(X, :), vixyz(Y, :), vixyz(Z, :), [1, nan]);
    data(:, :, ii) = reshape(i1, vdims(1:2));
    vixyz = (transform*structVol(1).mat) \ ixyzmm;
    i1 = ica_fuse_spm_sample_vol(structVol(1), vixyz(X, :), vixyz(Y, :), vixyz(Z, :), [1, nan]);
...
unitColor = ica_fuse_range(data(:))/64;
