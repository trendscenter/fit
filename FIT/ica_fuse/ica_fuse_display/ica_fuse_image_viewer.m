function ica_fuse_image_viewer(file_names, varargin)
%% Image viewer
%
% Inputs:
...
if (useGUI)
    ...
    file_names = ica_fuse_selectEntry('typeEntity', 'file', 'typeSelection', 'single', 'filter', '*.img;*.nii', 'filetype', 'image', 'title', 'Select file ...');
...
if (strcmpi(display_type, 'montage'))
    ...
    icasig = ica_fuse_applyDispParameters('image', icasig, convertToZ, returnValue, threshold);
    ...
    if (exist('labels', 'var'))
        title(labels, 'parent', axesH, 'color', UI_FG_COLOR, 'Horizontalalignment', 'center');
    end
    ...
    if strcmpi(TEXT_DISPLAY_SLICES_IN_MM, 'on')
        ...
        for nTextRows = 1:numImagesY
            ...
            for nTextCols = 1:numImagesX
                ...
                if textCount <= DIM(3)
                    txHandle(textCount) = text(xPos, yPos, num2str(round(slices_in_mm_new(textCount))), 'color', UI_FG_COLOR,  ...
                        'fontsize', fontSizeText, 'HorizontalAlignment', 'left', 'verticalalignment', 'bottom', ...
                        'FontName', UI_FONT_NAME, 'parent', axesH);
                end
...
elseif (strcmpi(display_type, 'render'))
    ...
    data = ica_fuse_applyDispParameters('image', data(:)', convertToZ, returnValue, threshold);
    ...
    dat   = struct(	'XYZ',	XYZ,...
        ...
        'dim',	V.dim(1:3)');
    global prevrend;
    rendfile = fullfile(fileparts(which('fusion.m')), 'ica_fuse_templates', 'render_single_subj.mat');
    prevrend.rendfile = rendfile;
    prevrend.col = [1 0 0; 0 1 0; 0 0 1];
    prevrend.brt = NaN;
    [rend, cmap] = ica_fuse_spm_render(dat, NaN, rendfile);
    rend = concatDat(rend);
    ...
    if (~isempty(cmap))
        ...
        if ((ica_fuse_get_matlab_version <= 2013) || strcmpi(version('-release'), '2014a'))
            title(maxLabel, 'parent', ch, 'color', UI_FG_COLOR, 'Horizontalalignment', 'center');
            xlabel(minLabel, 'parent', ch, 'color', UI_FG_COLOR, 'Horizontalalignment', 'center');
        else
...
popupH = ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'popup', 'position', editPos, 'string', char('Montage', 'Ortho', 'Render'), 'tag', ...
    'display_type', 'fontsize', UI_FONT_SIZE - 1, 'value', 1, 'callback', {@displayTypeCallback, InputHandle});
...
editPos(1) = editPos(1) + editPos(3) + xOffset;
editPos(3) = controlWidth;
editH = ica_fuse_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', Z_THRESHOLD, 'tag', 'threshold', 'fontsize', UI_FONT_SIZE - 1);
