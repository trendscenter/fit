function anovaResults = ica_fuse_anova(data, factors, varargin)
%% Compute N-way anova
%
% Inputs:
...
% anovaResults = ica_fuse_anova(y, {g1, g2, g3}, 'model', 'interaction', 'var_names', {'G1'; 'G2'; 'G3'});
% tbl = ica_fuse_anova_table(anovaResults);
%
    ...
    if (length(factors) > 1)
        ...
        for nComb = 1:size(all_comb, 1)
            ...
            for nM = 1:length(currentComb) - 1
                if (nM == 1)
                    contrastMatrix = ica_fuse_elem_by_elem_mult(anovaResults.factor(currentComb(nM)).contrastMatrix, ...
                        anovaResults.factor(currentComb(nM+1)).contrastMatrix);
                    temp_varname = [varNames{currentComb(nM)}, ' * ', varNames{currentComb(nM + 1)}];
                else
                    contrastMatrix = ica_fuse_elem_by_elem_mult(contrastMatrix, anovaResults.factor(currentComb(nM+1)).contrastMatrix);
...
% equations
constraint_matrix = ica_fuse_anova_constraint(modelMatrix, [anovaResults.factor(1:num_main_factors).cols_contrast_matrix]);
...
[beta_weights, line_fit] = ica_fuse_constrained_lse(contrastMatrix, data, constraint_matrix);
...
for nResults = 1:length(anovaResults.factor)
    ...
    if isempty(all_terms)
        ...
        %% Fit the model with the data using QR decomposition
        [temp_beta_weights, temp_line_fit] = ica_fuse_constrained_lse(contrastMatrix(:, all_terms), data, constraint_matrix(:, all_terms));
...
for nResults = 1:length(anovaResults.factor)
    ...
    pval = 1 - ica_fuse_spm_Fcdf(Fstat, anovaResults.factor(nResults).df, anovaResults.erdf);
