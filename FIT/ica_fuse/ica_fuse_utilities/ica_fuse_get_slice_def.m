function [parameters] = ica_fuse_get_slice_def(structVol, anatomicalView)
...
% 2. anatomicalView - anatomicalView like axial, sagittal or coronal
%
% Output:
...
if (~isempty(USE_DEFAULT_SLICES) && (USE_DEFAULT_SLICES == 1))
    if (~isempty(SLICESINMM))
        slicesInmm = SLICESINMM;
        if (isnumeric(slicesInmm))
            slicesInmm = num2str(slicesInmm);
        end
        
        %sr = strtrim( SLICE_RANGE );
        sr = deblank(slicesInmm);
