function [data, HInfo, XYZ] = ica_fuse_read_data(P, file_numbers, mask_ind, precisionType, varToLoad)
...
if (~isempty(mask_ind) && ischar(mask_ind))
    mV = ica_fuse_read_data(mask_ind);
    clear mask_ind;
...
if (~isempty(file_numbers))
    P = ica_fuse_rename_4d_file(P);
end

%% Parse extension
first_file = deblank(P(1, :));
first_file = ica_fuse_parseExtn(first_file);
...
elseif (strcmpi(extn, '.gii'))
    ...
    for nP = 1:size(P, 1)
        ...
        if (nP == 1)
            if (numel(temp) ~= length(temp))
                HInfo.DIM = [size(temp, 1), size(temp, 2), 1];
            else
                HInfo.DIM = [numel(temp), 1, 1];
            end
            HInfo.V.n(1) = 1;
            HInfo.V.dim = HInfo.DIM;
        end
        ...
        if (nP == 1)
            if strcmpi(precisionType, 'double')
                data = zeros(numel(temp), size(P, 1));
            else
                data = zeros(numel(temp), size(P, 1), 'single');
            end
        end
        data(:, nP) = temp(:);
    end
    ...
    if (numel(temp) ~= length(temp))
        data = squeeze(reshape(data, [HInfo.DIM(1:2), size(P, 1)]));
    end
...
if (~strcmpi(extn, '.gz'))
    ...
    if ~isempty(file_numbers)
        try
            ...
            V = ica_fuse_spm_vol(deblank(P(file_numbers, :)));
        catch
    ...
    if (V(1).private.hdr.dim(1) == 6)
        % read it as cifti
        data = cell(1, size(P, 1));
        for nF = 1:size(P, 1)
            tmp_dat = ft_read_cifti(deblank(P(nF, :)));
            tmp_dat = tmp_dat.dtseries;
    ...
    if (~isempty(mask_ind))
        voxel_coords = ica_fuse_get_voxel_coords(V(1).dim(1:3));
    ...
    for nVol = 1:length(V)
        ...
        if ((nVol == 1) && (nargout == 3))
            ...
            [tempData, XYZ] = ica_fuse_spm_read_vols(V(nVol));
            if (~isempty(mask_ind))
                tempData = tempData(mask_ind);
            end
        else
            if (~isempty(mask_ind))
                tempData = ica_fuse_spm_get_data(V(nVol), voxel_coords, 0);
            else
                tempData = ica_fuse_spm_read_vols(V(nVol));
            end
        end
    ...
    if (nargout == 3)
        XYZ = ica_fuse_get_voxel_coords(V(1).dim(1:3));
        RCP = XYZ;
        RCP(4, :) = 1;
        XYZ = (V(1).mat(1:3, :)*RCP);
    end
...
for nI = 1:length(inds)
    ...
    [dat{nI}, hdr, tmpV] = ica_fuse_read_gzip_nii(ufNames{nI}, 'timepoints', nums, 'mask', mask_ind);
    if (nI == 1)
        V = tmpV;
    end
end
