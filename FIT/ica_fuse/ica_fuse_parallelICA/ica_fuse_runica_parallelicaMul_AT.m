...
function [weights,sphere,activations,bias,signs,lrates,y] = ica_fuse_runica_parallelicaMul_AT(data,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6,p7,v7,p8,v8,p9,v9,p10,v10,p11,v11,p12,v12,p13,v13,p14,v14)

if nargin < 5
    help ica_fuse_runica_parallelicaMul_new
    return
end
...
while step(1) < maxsteps | step(2) < maxsteps
        ...
        if wts_blowup(1) | isnan(change(1))|isinf(change(1)),  % if weights blow up,
            ...
            if verbose,
                if step(1) > 2,
                    if ~extended,
                        fprintf(...
                            'Dataset 1 step %d - lrate %5f, wchange %7.6f, angledelta %4.1f deg\n', ...
                            step(1),lrate(1),change(1),degconst*angledelta(1));
                    else
                        fprintf(...
                            'Dataset 1 step %d - lrate %5f, wchange %7.6f, angledelta %4.1f deg, %d subgauss\n',...
                            step(1),lrate(1),change(1),degconst*angledelta(1),(ncomps(1)-sum(diag(signs{1})))/2);
                    end
                elseif ~extended
                    fprintf(...
                        'Dataset 1 step %d - lrate %5f, wchange %7.6f\n',step(1),lrate(1),change(1));
                else
                    fprintf(...
                        'Dataset 1 step %d - lrate %5f, wchange %7.6f, %d subgauss\n',...
        ...
        if wts_blowup(2) | isnan(change(2)) | isinf(change(2)),  % if weights blow up,
            ...
            if verbose,
                if step(2) > 2,
                    if ~extended,
                        fprintf(...
                            'Dataset 2 step %d - lrate %5f, wchange %7.6f, angledelta %4.1f deg\n', ...
                            step(2),lrate(2),change(2),degconst*angledelta(2));
                    else
                        fprintf(...
                            'Dataset 2 step %d - lrate %5f, wchange %7.6f, angledelta %4.1f deg, %d subgauss\n',...
                            step(2),lrate(2),change(2),degconst*angledelta(2),(ncomps(2)-sum(diag(signs{2})))/2);
                    end
                elseif ~extended
                    fprintf(...
                        'Dataset 2 step %d - lrate %5f, wchange %7.6f\n',step(2),lrate(2),change(2));
                else
                    fprintf(...
                        'Dataset 2 step %d - lrate %5f, wchange %7.6f, %d subgauss\n',...
    ...
    if (step(1)>2 & step(2)>2) & ( ~stopsign(1) | ~stopsign(2))
        
        % A matrix of data1 (smri)
        mx = (weights{1}' \ dewhiteM{1}')';%num_sub*num_IC
        num_sub = size(mx,1);
        num_vox_fmri = size(data2,2);
        % A matrix of data2 (fmri)
        sx = (weights{2}' \ dewhiteM{2}')';%%TC matrix for fmri all_time_points*num_IC
        num_IC = size(sx,2);
        %  build A : num_sub*num_IC (distance^2) matrix of data2 (fmri) basesd on distance^2 between Sagg
        %  and Ssub
        S_group = weights{2}*data2;
        ...
        for sub = 1:num_sub%%calculate source for all subjects, ie back reconstruction
            ...
            temp_whiteM = reshape(temp,num_IC,num_vox_fmri);
            S_sub(sub,:,:) = weights{2}*temp_whiteM; 
        end
        D1 = zeros(num_sub,num_IC);%D2 is the constructed A for data2 like mx for data1
        for i = 1:num_IC
            for j = 1:num_sub
                temp = S_sub(j,:,:);
                temp_S_sub = reshape(temp,num_IC,num_vox_fmri);
                D1(j,i) = sum((S_group(i,:)-temp_S_sub(i,:)).^2);
            end
        end
        D2 = D1./norm(D1);
        % calculate the correlation of all componentsts   % match tow modality components
        [Corr_matrix]=abs(ica_fuse_corr(mx,D2));
        ...
        if ~ isempty(ix)
            ...
            for Cons_com=index(1:length(ix)); % constraned componenets
                ...
                % Updata the weights
                a=mx(:,maxcol(Cons_com))';u=D2(:,maxrow(Cons_com))'; %
                b=cov(u,a);  b=b(2); tmcorr=b/std(a)/std(u);
                ...
                if ~stopsign(2) % & ~Overindex2
                    for i = maxrow(Cons_com)
                        for j = 1:num_sub
                            temp = S_sub(j,:,:);
                            temp_S_sub = reshape(temp,num_IC,num_vox_fmri);
                            tempDj(j,:) = abs((S_group(i,:)-temp_S_sub(i,:)));
                        end
                    end
                    tempWZ = 2*tempDj*data2';
                    deltaf=(comterm*( (a'-mean(a'))' + b/var(u')*(mean(u') -u')'))*tempWZ;% 1st order derivative
                    P=deltaf./norm(deltaf) ;% (H2*deltaf')';
%                     deltaf1=comterm*(a-mean(a)+b*(mean(u)-u)/var(u));% 1st order derivative
%                     P1=deltaf1./norm(deltaf1);
%                     alphak(2)=findsteplength1(-tmcorr^2,-deltaf1,u,a,alphak(2),P1,0.0001,0.999);
                    aweights_temp=Crate(2)*alphak(2)*P; %
                    weights{2}(maxrow(Cons_com),:)=weights{2}(maxrow(Cons_com),:) + aweights_temp;
...
%             sx = (weights{2}' \ dewhiteM{2}')';
            ...
            D2 = D1./norm(D1);
            a=mx(:,maxcol(index(1))); b=D2(:,maxrow(index(1)));
    ...
    %     if (max(step(1)) == (maxsteps)  || max(step(2)) == (maxsteps))
    %         warning('!!!Reached max steps. Please change the setup options and restart parallel ica.');
    %     end
    %
...
if ((change(1) > nochange) || (change(2) > nochange))
    if (max(step(1)) == (maxsteps)  || max(step(2)) == (maxsteps))
        warning('!!!Reached max steps. Please reduce the constrained components in setup options and restart parallel ica.');
    end
end
