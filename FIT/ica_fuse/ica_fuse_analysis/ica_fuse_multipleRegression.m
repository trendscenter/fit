...
for nDataSets = 1:num_DataSets
    ...
    % Regressor matrix
    [X] = ica_fuse_modelX(model(rowStart:rowEnd, :), numTimePoints(nDataSets), detrendNumber);
    ...
    for nRegress = 1:num_Regress
        ...
        [partialCorrSlopes(countDataSet), partial_rsquare] = ica_fuse_regress(currentTc, X(:, nRegress));
