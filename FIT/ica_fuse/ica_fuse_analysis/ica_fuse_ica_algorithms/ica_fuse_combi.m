function [W, Wefica, Wwasobi, ISRwa, ISRef, metoda]= ica_fuse_combi(X,AR_order,ortho)
%
...
[Wefica, ISRef]=ica_fuse_efica(x);
[Wwasobi, Wsobi,ISRwa]= ica_fuse_ewasobi(x,10,0.9);
ISR1=sum(ISRef,2);
ISR2=sum(ISRwa,2);
imin=min([ISR1 ISR2]);
if imin(1)<imin(2)
...
while length(ostatni)>1
    [Wefi, ISRef0] = ica_fuse_efica(y);
    [Wwa,AOL_init,ISRwa0]= ica_fuse_ewasobi(y,10,0.9);
    ISR1=sum(ISRef0,2);
    ISR2=sum(ISRwa0,2);
    imin=min([ISR1 ISR2]);
    if imin(1)<imin(2)
