function ica_fuse_spm_reslice(P,flags)
...
if ischar(P),    P = ica_fuse_spm_vol(P); end

%def_flags        = ica_fuse_spm_get_defaults('realign.write');
def_flags.prefix = 'r';
if nargin < 2
    flags = def_flags;
else
...
for i = 1:numel(P)
    ...
    if read_vol
        if ~isfinite(flags.interp)
            v = abs(kspace3d(ica_fuse_spm_bsplinc(P(i),[0 0 0 ; 0 0 0]'),P(1).mat\P(i).mat));
            for x3 = 1:P(1).dim(3)
                if flags.mean
                    Integral(:,:,x3) = ...
                        Integral(:,:,x3) + ...
        ...
        if write_vol
            VO         = P(i);
            VO.fname   = ica_fuse_spm_file(P(i).fname, 'prefix',flags.prefix);
            VO.dim     = P(1).dim(1:3);
            VO.dt      = P(i).dt;
            VO.pinfo   = P(i).pinfo;
            VO.mat     = P(1).mat;
            VO.descrip = 'spm - realigned';
            VO = ica_fuse_spm_write_vol(VO,v);
        end
...
if flags.mean
    ...
    PO          = P(1);
    PO.fname    = ica_fuse_spm_file(P(1).fname, 'prefix','mean');
    PO          = rmfield(PO,'pinfo');
    PO.pinfo    = [max(max(max(Integral)))/32767 0 0]';
    PO.n        = [1 1];
    PO.descrip  = 'spm - mean image';
    PO.dt       = [ica_fuse_spm_type('int16') ica_fuse_spm_platform('bigend')];
    ica_fuse_spm_write_vol(PO,Integral);
end
