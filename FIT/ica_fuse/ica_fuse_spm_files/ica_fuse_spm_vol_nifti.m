function V = ica_fuse_spm_vol_nifti(fname,n)
%  Get header information for a NIFTI-1 image.
%  FORMAT V = ica_fuse_spm_vol_nifti(P)
%  P - filename.
...
% John Ashburner
% $Id: ica_fuse_spm_vol_nifti.m 1143 2008-02-07 19:33:33Z spm $


if nargin<2,  n = [1 1];      end;
if ischar(n), n = str2num(n); end;
N  = ica_fuse_nifti(fname);
n  = [n 1 1];
n  = n(1:2);
...
off = (n(1)-1+dm(4)*(n(2)-1))*ceil(ica_fuse_spm_type(dt(1),'bits')*dm(1)*dm(2)/8)*dm(3) + N.dat.offset;
