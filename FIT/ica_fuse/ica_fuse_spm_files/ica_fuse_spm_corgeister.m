function resliced_files = ica_fuse_spm_corgeister(coregRefImage, coregSourceImage, otherImages, outDir)
...
tmp_files = ica_fuse_rename_4d_file(coregSourceImage);

coregSourceImage = deblank(tmp_files(1, :));

otherImages = ica_fuse_rename_4d_file(otherImages);

%% Coregister
% The above code is from spm_config_coreg function
disp(['Coregistering ', coregSourceImage, ' to ', coregRefImage, ' ...']);
x  = ica_fuse_spm_coreg(coregRefImage, coregSourceImage, eoptions);
M  = inv(ica_fuse_spm_matrix(x));

%% Get orientation
PO = strvcat(coregSourceImage, otherImages);
MM = zeros(4,4,size(PO,1));
for j = 1:size(PO,1),
    MM(:, :, j) = ica_fuse_spm_get_space(deblank(PO(j, :)));
end
...
coregSourceImage_tmp = ica_fuse_parseExtn(coregSourceImage);
