function [strout,R2] = ica_fuse_spm_str_manip(strin,options)
% miscellaneous string manipulation options
% FORMAT string_out = ica_fuse_spm_str_manip(string_in,options)
...
%                          are replaced by './'.
%                          eg. ica_fuse_spm_str_manip('/dir1/dir2/file.img','a16') would
%                          produce '../dir2/file.img'.
%       'v'              - delete non valid filename characters
%                          Valid are '.a..zA..Z01..9_-: ' & filesep
%       'p'              - canonicalise pathname (see ica_fuse_spm_select('CPath',strin))
...
% John Ashburner
% $Id: ica_fuse_spm_str_manip.m 1143 2008-02-07 19:33:33Z spm $
...
while (~isempty(options))
    ...
    for i=1:prod(size(strout))
        ...
        case 's'    % Strip off '.img', '.hdr' or '.mat' suffixes
            ...
            if (l > 4)
                if (strcmp(str((l-3):l),'.img') | ...
                    ...
                    str = ica_fuse_spm_str_manip(str, 'r');
                end
            end
        ...
        case 'p'
            str = strvcat(ica_fuse_spm_select('CPath',cellstr(str)));
