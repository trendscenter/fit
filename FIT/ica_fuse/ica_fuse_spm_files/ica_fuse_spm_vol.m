function V = ica_fuse_spm_vol(P)
% Get header information etc for images.
% FORMAT V = ica_fuse_spm_vol(P)
...
%       V.dt    - A 1x2 array.  First element is datatype (see ica_fuse_spm_type).
...
%
% Note that ica_fuse_spm_vol can also be applied to the filename(s) of 4-dim
...
% John Ashburner
% $Id: ica_fuse_spm_vol.m 2237 2008-09-29 17:39:53Z guillaume $
...
[pth,nam,ext,n1] = ica_fuse_spm_fileparts(deblank(p));
p = fullfile(pth,[nam ext]);
n = str2num(n1);
if ~ica_fuse_spm_existfile(p),
...
switch ext,
    ...
    case {'.img','.IMG'},
        if ~ica_fuse_spm_existfile(fullfile(pth,[nam '.hdr'])) && ~ica_fuse_spm_existfile(fullfile(pth,[nam '.HDR'])),
...
if isempty(n),
    V = ica_fuse_spm_vol_nifti(p);
else
    V = ica_fuse_spm_vol_nifti(p,n);
end;
    
if isempty(n) && length(V.private.dat.dim) > 3
    V0(1) = V;
    for i = 2:V.private.dat.dim(4)
        V0(i) = ica_fuse_spm_vol_nifti(p, i);
    end
    V = V0;
end
