function [spmData] = ica_fuse_loadSPM_new(varargin)
...
try
    ...
    if strcmp(check_spm_design_matrix, 'yes')
        ...
        spmMat = ica_fuse_includeFields_struct(spmMat, fieldsReq1);
        clear fieldsReq1;
        ...
        if isfield(Sess_new, 'U')
            ...
            if ~isempty(Sess_new(1).U)
                Sess_new = ica_fuse_includeFields_struct(Sess_new, {'U'});
                fieldsReq1 = {'name', 'ons'};
                % include only the required fields
                for ii = 1:length(Sess_new)
                    Sess_new(ii).U =  ica_fuse_includeFields_struct(Sess_new(ii).U, fieldsReq1);
                end
                clear fieldsReq1;
            end
