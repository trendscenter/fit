function varargout = mygift_icaAlgorithm(ica_algorithm, data, sesInfo, ICA_Options)
...
if (strcmpi(modalityType, 'fmri'))
    ...
    icaAlgo = char('Infomax','Fast ICA', 'Erica', 'Simbec', 'Evd', 'Jade Opac', 'Amuse', ...
        ...
        'IVA-L', 'Sparse ICA-EBM', 'IVA-L-SOS', 'IVA-L-SOS-Adaptive', 'Constrained ICA EBM', 'pmlJICA');% 'Constrained_ICA_EBM', 'Adaptive_Constrained_ICA_EBM');
...
if (nargin > 0 && nargin <= 4)
    ...
    % end for checking
    selected_ica_algorithm
    switch(lower(selected_ica_algorithm))
        
        case 'infomax'
            %% Infomax
            disp('Inside the infoMax.....')
            ...
            [W, sphere, icasig_tmp] = icatb_infomax(data, ICA_Options{1:length(ICA_Options)});  %%% also infomax ICA and jICA (infomax)
            W = W*sphere;
            icasig_tmp = W*data;
            A = pinv(W);
        ...
        case 'pmljica'
            ...
            [W, sphere, ~] = mygift_pmljICA(data, sesInfo, ICA_Options);  %%% also infomax ICA and jICA (infomax)
            W = W*sphere;
            icasig_tmp = W*data;
            A = pinv(W); 
        ...
        case 'sdd ica'
            ...
            [W, sphere, icasig_tmp] = icatb_sdd_ica(data, ICA_Options{1:length(ICA_Options)});
            W = W*sphere;
            icasig_tmp = W*data;
            A = pinv(W);
        ...
        case {'fbss', 'erbm'}
            ...
            W = icatb_fbss(data, ICA_Options{:});  %%% use ICA-EBM (icatb_ica_ebm) to provide the initial guess
            icasig_tmp = W*data;
            A = pinv(W);
        ...
        case 'constrained ica ebm'
            ...
            [data, dewhiteM] = (mygift_calculate_pca(data', size(ref_data, 2)));
            data = data';
...
function [W, A, SR]  = correct_sign(W, X)  %%%% Not for Infomax
...
function wM = doSecondPCAStep(data)   %%%% Not for Infomax, for the specific algorithm
...
[groupPCASig, dewhiteM] = mygift_calculate_pca(data, numComp, 'type', 'mpowit', 'whiten', true);
wM = pinv(dewhiteM);



function ICA_Options = updateICAOpts(data, ICA_Options, msgStr)  %%% %%%% Not for Infomax, for the specific algorithm
