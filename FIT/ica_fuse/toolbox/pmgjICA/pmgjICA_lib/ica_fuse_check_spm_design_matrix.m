function ica_fuse_check_spm_design_matrix(designMatrix, numOfSub, numOfSess, diffTimePoints, spmMatFlag, inputFile)
...
if ~isempty(designMatrix(1).name)
    ...
    if strcmpi(spmMatFlag, 'same_sub_same_sess')
        ...
        % do spm checking
        [spmData] = ica_fuse_loadSPM_new('spmName', deblank(designMatrix(1).name), 'countTimePoints', ...
    ...
    elseif strcmpi(spmMatFlag, 'same_sub_diff_sess')
        ...
        for ii = 1:numOfSub
            ...
            % loop over sessions
            for jj = 1:numOfSess
                [spmData] = ica_fuse_loadSPM_new('spmName', deblank(designMatrix(ii).name), ...
