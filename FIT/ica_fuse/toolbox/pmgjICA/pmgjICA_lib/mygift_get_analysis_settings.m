function varargout = mygift_get_analysis_settings(sesInfo, perfType)
...
if (~isfield(sesInfo.userInput, 'pca_opts'))
    if (isfield(sesInfo.userInput, 'covariance_opts'))
        ...
        pca_opts_user_specified = mygift_pca_options(pcaType);
    end
else
...
pca_opts_user_specified = mygift_pca_options(pcaType, pca_opts_user_specified, 'off');
...
algoNames = cellstr(mygift_icaAlgorithm);
