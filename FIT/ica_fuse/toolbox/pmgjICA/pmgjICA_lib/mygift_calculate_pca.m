function varargout = mygift_calculate_pca(files, numComp, varargin)
...
% %   must be specified in a data structure. Please see ica_fuse_pca_options(pcaType) for more details.
...
% %   data = randn(3000, 200);
% %   numComp = 5;
% %   [V, Lambda] = icatb_calculate_pca(data, numComp, 'whiten', false);
% %   [whitesig, dewhiteM, Lambda, V, whiteM] = icatb_calculate_pca(data, numComp, 'whiten', true);
% %
% % b. Files in NII format
% % files = {'C:\ns1.nii', 'C:\ns2.nii', 'C:\ns3.nii'};
% % mask = 'myMask.nii';
% % [V, Lambda] = icatb_calculate_pca(files, numComp, 'mask', mask, 'whiten', false);
% %
% % c. Files in MAT format
% % files = {'C:\ns1.mat', 'C:\ns2.mat', 'C:\ns3.mat'};
% % [V, Lambda] = icatb_calculate_pca(files, numComp, 'whiten', false, 'vartoload', 'pcasig');
% %
% % d. Best PCA:
% % files = {'C:\ns1.nii', 'C:\ns2.nii', 'C:\ns3.nii'};
% % mask = 'myMask.nii';
...
if (~exist('pcaType', 'var') || strcmpi(pcaType, 'best'))
    ...
    chkPCA = ica_fuse_check_pca_opts(chkPCA);
...
if (isnumeric(files) && strcmpi(pcaType, 'stp'))
    %disp('Numeric data is passed. Using EVD instead of STP ...');
    pcaType = 'evd';
...
if (~exist('pca_options', 'var'))
    pca_options = mygift_pca_options(pcaType);
end

pca_options = mygift_pca_options(pcaType, pca_options, 'off');
...
if (isnumeric(files) || strcmpi(stack_data, 'yes'))
    ...
    disp('Always program is here for my case...............')
    ...
    if (removeMean)
        disp('Inside Remove mean,......')
        data = ica_fuse_remove_mean(data, 0);
    end
    disp('Second data reduction from files.....');
    size(data)  %% % 50485  60 for the example data if numofComp 20
    ...
    switch (lower(pcaType))
        ...
        case 'svd'
            ...
            [V, Lambda] = ica_fuse_svd(data, numComp, 'solver', pca_options.solver, 'verbose', verbose);
        ...
        case {'empca', 'expectation maximization'}
            ...
            [V, Lambda] = ica_fuse_calculate_em_pca(data, Vi, 'tolerance', tol, 'max_iter', pca_options.max_iter, 'verbose', verbose);
    end
    ...
    switch (lower(pcaType))
        ...
        case {'evd', 'standard'}
            ...
            if (rows > cols)
                ...
                [V, Lambda] = ica_fuse_eig_symm(cov_m, dims(2), 'num_eigs', numComp, 'eig_solver', eig_solver, 'verbose', verbose);
                checkEig(Lambda);
                if (whitenMatrix)
                    [whiteM, dewhiteM] = get_pca_info(V, Lambda);
        ...
        case 'stp'
            ...
            [whitesig, S0, Lambda, V] = stp(files, numComp, dims, 'mask', mask, 'preproc_type', preproc_type, 'precision', precisionType, 'varToLoad', varToLoad, ...
...
if (rows >= cols)
    ...
    [V, Lambda] = ica_fuse_eig_symm(cov_m, size(cov_m, 1), 'num_eigs', numComp, 'eig_solver', eig_solver, 'verbose', verbose);
else
...
if (numComp >= ceil(0.6*minDims))
    ...
    [V, D] = mygift_calculate_pca(data, numComp, 'type', 'evd', 'verbose', 0, 'whiten', 0);
...
try
    ...
    [dd, S, D] = stp(files, numComp, dims, 'mask', mask, 'preproc_type', preproc_type, 'precision', precisionType, 'varToLoad', varToLoad, ...
        'numGroups', numGroups, 'interComp', interComp);
    Dold = diag(D);
catch
...
if (~isnumeric(files))
    ...
    if (strcmpi(extn, '.mat'))
        ...
        tp = 0;
        for nF = 1:length(files)
            tmp = ica_fuse_rename_4d_file(files{nF});
            tp = tp + size(tmp, 1);
        end
        dims = [length(mask_ind), tp];
    end
...
if (ica_fuse_get_matlab_version < 2013)
...
if ((nLoops + 1) < numPairs)
    ...
    for nF = 1:nLoops
        ...
        tmp = ica_fuse_read_data(files, [], mask_ind(startLoop:endLoop), precisionType, varToLoad);
...
[S, Lambda] = ica_fuse_eig_symm(cov_m, voxels, 'num_eigs', numComp, 'eig_solver', 'selective');
...
endTp = 0;
for nF = 1:length(files)
    tmp = ica_fuse_read_data(files{nF}, [], mask, precisionType, varToLoad);
    tmp = squeeze(tmp);
    tmp = reshape(tmp, size(tmp, 1), size(tmp, 2)*size(tmp, 3));
    startTp = endTp + 1;
