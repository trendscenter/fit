function [fileNames, zipFileName, files_to_zip] = ica_fuse_saveICAData(outfile, icasig, A, mask_ind, numOfIC, HInfo, dataType, ...
...
[modalityType, dataTitle, compSetFields] = ica_fuse_get_modality;
...
lastUnderScore = ica_fuse_findstr(outfile,'_');
...
if strcmpi(modalityType, 'eeg')
    ...
    ica_fuse_save(fileNames{1}, compSetFields{:});
    zipFileName = {};
    files_to_zip = {};
    return;
end
...
elseif isa(A, 'complex_data')
    ...
    ica_fuse_write_vol(V, getfield(A, 'firstField'));
    ...
    ica_fuse_write_vol(V, getfield(A, 'secondField'));
...
for i=1:numOfIC
    % return file index
    [fileIndex] = ica_fuse_returnFileIndex(i);
    ...
    elseif isa(data, 'complex_data')
        ...
        V.fname = fullfile(outputDir, V.fname);
        % write image
        ica_fuse_write_vol(V, getfield(data, 'firstField'));
        ...
        V.fname = fullfile(outputDir, V.fname);
        % write image
        ica_fuse_write_vol(V, getfield(data, 'secondField'));
