function [files, designMatrix, numOfSub, numOfSess, dataSelMethod, diffTimePoints, spmMatFlag, bids_info]  = mygift_dataSelection(...
    inputFile, inputdataMat, outputDir, output_prefix, read_complex_file_naming, read_complex_images)


%%%% Need to check other functions, later
%%%% Now only dataSelMethod=1;
...
if isempty(inputFile)
    ...
    if (~strcmpi(modalityType, 'smri'))
        ...
        %%%% 'Is your data stored in one group folder (Yes - Includes BIDS
        %%%% format)?'; :  Answer: yes, in the default example

    
        
        popupAnswer = 1; %%% 'yes': data stored in one group folder
...
if (dataSelMethod == 1)
    ...
    if isempty(inputFile)
        ...
        use_bids = 0;  %% 'BIDS format?': no
        
        
        data_setDir = 'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor';
...
if (dataSelMethod == 1 && ~dataMatrix)
    ...
    if isempty(inputFile)
        ...
        if ~isempty(data_setDir)
            ...
            answer{1}=filterTextStr;
            answer{2}='No'; %%%%% 'Are session folders inside subject folders?';: Yes or No
            answer{3}='';
    ...
    [files, numOfSub, numOfSess, selected_data_sets, data_folders] = mygift_get_sub_data(data_setDir, filePattern, flagFolder);
    % %     disp('Checking.....')
    % %     files.name
    % %     numOfSub
    % %     numOfSess
    ...
    if strcmpi(spmMatFlag, 'diff_sub_diff_sess')  %%%% Not application for the example data
        ...
        for ii = 1:numOfSub
            ...
            if isempty(tempdesignMat)
                try
                    tempdesignMat = mygift_get_sub_data(currentSub, spmDesignFilter, 'data_in_subject_folder', 0);
                catch
    ...
    for nSub = 1:numOfSub
        ...
        for nSess = 1:numOfSess
            ...
            currentFileNum = fileNumbers;  %%%% [] for the example data
...
elseif (dataSelMethod==1 && dataMatrix)
    %%% get the data_sets automatically
    %%[files, numOfSub, numOfSess, selected_data_sets, data_folders] = mygift_get_sub_data(inputdataMat);
    % %     files.name
    % %     ....
    % %     'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor\sub03_vis\nsrstim_219.img'
    % %     'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor\sub03_vis\nsrstim_220.img'
    % %     numOfSub =
    % %      3
    % %      numOfSess =
    ...
    % %      selected_data_sets =
    % %      3×67 char array
    % %      'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor\sub01_vis'
    % %      'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor\sub02_vis'
    % %      'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor\sub03_vis'
    % %      ans =
    % %      'C:\myproject\utils\gift_noGUI\example_subjects\visuomotor\sub01_vis'
    % %      ans =
...
elseif (dataSelMethod == 3)
    ...
    for nSub = 1:numOfSub
        ...
        for nSess = 1:numOfSess
            ...
            if (size(fileP, 1) == 1)
                [pathstr, fp, extn] = fileparts(icatb_parseExtn(deblank(fileP)));
...
if (strcmpi(modalityType, 'eeg'))
    ...
    ica_fuse_check_spm_design_matrix(designMatrix, numOfSub, numOfSess, diffTimePoints, spmMatFlag, inputFile);
end
