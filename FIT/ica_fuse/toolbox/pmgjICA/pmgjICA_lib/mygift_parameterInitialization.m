function [sesInfo] = mygift_parameterInitialization(sesInfo, statusHandle)
...
outputDir = sesInfo.userInput.pwd; % get the output directory information

if ~exist('statusHandle', 'var')
    statusHandle = [];
end

[modalityType, dataTitle] = mygift_get_modality;
...
if (strcmpi(modalityType, 'fmri'))
    ...
    if (useTemporalICA)
        if (strcmpi(algorithmName, 'iva-gl') || strcmpi(algorithmName, 'moo-icar') || ica_fuse_string_compare(algorithmName, 'constrained') || ...
...
if isfield(sesInfo.userInput, 'diffTimePoints') || isempty(sesInfo.userInput.files)
    countFiles = sesInfo.userInput.diffTimePoints; % different time points
else
    % get the count of the files
    [countFiles] = mygift_get_countTimePoints(sesInfo.userInput.files);
end
...
try
    if isfield(sesInfo.userInput.HInfo.V, 'gifti') && ~isempty(sesInfo.userInput.files)
        [ddd, gfn] = fileparts(sesInfo.userInput.files(1).name(1,:));
        sesInfo.icaOutputFiles = ica_fuse_getOutputFileNames([sesInfo.userInput.prefix, '_', gfn], sesInfo.numComp, sesInfo.numOfSub, sesInfo.numOfSess, sesInfo.write_analysis_steps_in_dirs, '.gii');
    else
        sesInfo.icaOutputFiles = ica_fuse_getOutputFileNames(sesInfo.userInput.prefix, sesInfo.numComp, sesInfo.numOfSub, sesInfo.numOfSess, sesInfo.write_analysis_steps_in_dirs);
    end
...
[sesInfo, complexInfo] = mygift_name_complex_images(sesInfo, 'read');

if ~strcmpi(modalityType, 'eeg') && ~isempty(sesInfo.userInput.files)
    % return HInfo
    tmpMaskFile = fullfile(outputDir, [sesInfo.userInput.prefix, 'Mask.nii']);
    if (exist(tmpMaskFile, 'file') == 2)
        [V, sesInfo.HInfo] = mygift_returnHInfo(tmpMaskFile);  %%%% io data function file
    else
        [V, sesInfo.HInfo] = mygift_returnHInfo(sesInfo.userInput.files(1).name(1, :));  %%%% io data function file
    end
    clear V;
else
...
if ((sesInfo.numOfSub*sesInfo.numOfSess > 1) && (sesInfo.numReductionSteps == 1))
    tmpBackReconOpts = lower({'spatial-temporal regression', 'GIG-ICA', 'Constrained ICA'});
    chkBackRecon = strmatch(lower(backReconType), tmpBackReconOpts, 'exact');
    if (isempty(chkBackRecon))
        backReconType = 'spatial-temporal regression';
    else
        backReconType =  tmpBackReconOpts{chkBackRecon(1)};
    end
end
...
sesInfo.userInput = ica_fuse_check_pca_opts(sesInfo.userInput);
...
sesInfo = mygift_dataReductionSetup(sesInfo);

%check to make sure valid parameters
mygift_parameterErrorCheck(sesInfo);
