function inputText = mygift_define_parameters(varargin)
...
global EXPERIMENTAL_TR;  %% %% Experimental TR in seconds     EXPERIMENTAL_TR = 1;
...
% % % 4 - GICA
% % BACKRECON_DEFAULT = 4;
% % %% Options for scaling are in ica_fuse_scaleICA
...
preproc_default = matchString(PREPROC_DEFAULT, ica_fuse_preproc_data, 1);
pca_default = matchString(PCA_DEFAULT, mygift_pca_options, 1);
...
scaleOptions = ica_fuse_scaleICA;
...
if (~strcmpi(modalityType, 'smri'))
    ...
    inputText(numParameters).answerString = char(ica_fuse_preproc_data); %ce030525
...
inputText(numParameters).answerString = char(mygift_pca_options);
inputText(numParameters).uiType = 'popup';
inputText(numParameters).dataType = 'string';
...
inputText(numParameters).answerString = mygift_icaAlgorithm;
inputText(numParameters).uiType = 'popup';
inputText(numParameters).dataType = 'numeric';
