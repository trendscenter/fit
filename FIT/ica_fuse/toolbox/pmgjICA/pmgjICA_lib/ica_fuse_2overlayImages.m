function [im,maxICAIM,minICAIM,minInterval,maxInterval] = ica_fuse_2overlayImages(icasig,structuralImage,icaDIM,structDIM,imageValues, imageScale)
%  [im,maxICAIM,minICAIM,minInterval,maxInterval] = ica_fuse_overlayImages(icasig,structuralImage,icaDIM,structDIM)
%--------------------------------------------------------------------------
...
%       imageValues: 1='positive and negative', 2='positive', 3='absolute value', 4='negative'
...
for i=1:numToOverlay
    ...
    elseif(imageValues==4)
        ...
        %ica_fuse_error('Invalid Image Value Parameter',InfoCell);
...
for i =1:numToOverlay
    ...
    unitColor = ica_fuse_range([tmp_min_ica_im, tmp_max_ica_im])/(min([256 64*(numToOverlay+1)])/(numToOverlay+1));
    ...
    multi = tmp_max_ica_im/ica_fuse_range([tmp_min_ica_im, tmp_max_ica_im]);
...
%scale structural
unitColor = ica_fuse_range([minInterval maxInterval])*(numToOverlay+1)/min([256 64*(numToOverlay+1)]);
newMin = maxInterval*numToOverlay;
newMax = maxInterval*(1+numToOverlay);
structFlat = ica_fuse_scaleImage(newMin,newMax,structFlat,'anatomical'); %ce042525
