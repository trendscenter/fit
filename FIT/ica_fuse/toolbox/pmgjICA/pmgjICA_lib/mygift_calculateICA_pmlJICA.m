function [sesInfo] = mygift_calculateICA_pmlJICA(sesInfo, reduction_sep, dataMat, statusHandle)
...
if ~exist('dataMat', 'var') || isempty(dataMat)
    disp('!!Working for the two Stage Group ICA algorithm [1]!!')
    disp('Data read from the reduction mat files.')
    disp('Not work for temporal and IVA')
    dataMat=[];

    %%load(sesInfo.userInput.datapath);
end
...
global NUM_RUNS_GICA;  %%%  1 default
...
if (which_analysis == 2)  %% this is for the ICASSO algorithm
...
if (isempty(ica_fuse_findstr(lower(algorithmName),'iva')) && ~strcmpi(algorithmName, 'moo-icar') && ~ica_fuse_string_compare(algorithmName, 'constrained'))
    if (which_analysis == 1)
...
elseif (~isempty(ica_fuse_findstr(lower(algorithmName),'iva')))
...
if (~strcmpi(algorithmName, 'moo-icar') && ~ica_fuse_string_compare(algorithmName, 'constrained'))
    data = mygift_getDataForICA(sesInfo, algorithmName, dataMat, reduction_sep, statusHandle);  %%% data read from 'tst_pca_r2-1.mat'  %%% nCompx50485
else
    parICAReference(sesInfo, algorithmName, parallel_info, dataMat, statusHandle);
...
if strcmpi(algorithmName, 'semi-blind infomax')
    ...
    ICA_Options = ica_fuse_sbica_options(ICA_Options, dewhiteM);
...
if (isempty(ica_fuse_findstr(lower(algorithmName),'iva')))
    ...
    if (which_analysis == 1)
        %disp('which analysis empty iva');
        %which_analysis  %%% 1 
        
        fprintf('\n');
        disp(['Number of times ICA will run is ', num2str(numRuns)]);  %%% Run 1 / 1
        
        % Loop over number of runs
        for nRun = 1:numRuns  %%%% Why multiple run
            ...
            fprintf('\n');
            %%%%disp('Program is here which analysis-1')  %%% here
            disp('Data during calling.....')
            size(data)  %%% 20   50485
            ...
            %%%% %%% always think that this is called after reduction
            %%%% according to the toolbox
            [icaAlgo, W, A, icasig] = mygift_icaAlgorithm(algorithmName, data, sesInfo, ICA_Options);

            %%disp('algorithm Check ....')
            %%icaAlgo  %%% all algorithm name
            size(W)  %%% 20 20
            size(A)  %%% 20 20
            size(icasig)  %%% 20       50485
    ...
    elseif (which_analysis == 2)
        ...
        if (strcmpi(parallel_info.mode, 'serial') || parallelCluster)
            ...
            % Only supports serial processing for now
%             if strcmpi(parallel_info.mode, 'serial')
                sR = ica_fuse_icassoEst(icasso_opts.sel_mode, data, icasso_opts.num_ica_runs, 'numOfPC', numOfIC, 'algoIndex', sesInfo.algorithm, ...
...
%             end
        ...
        if (~useTemporalICA)
            mygift_save(icassoResultsFile, 'iq', 'A', 'W', 'icasig', 'sR', 'algorithmName', 'metric_Q');
        else
            temporal_icasig = icasig;
            clear icasig;
            mygift_save(icassoResultsFile, 'iq', 'A', 'W', 'temporal_icasig', 'sR', 'algorithmName', 'metric_Q');
        end
    ...
    elseif (which_analysis == 4)
        ...
        parfor nRI = 1:sesInfo.cross_isi_opts.num_ica_runs
            ...
            [icaAlgoxx, WR(:, :, nRI), Axx, icxx] = mygift_icaAlgorithm(algorithmName, data, ICA_Options);
        end
        ...
        mygift_save(criResultsFile, 'A', 'W', 'icasig', 'WR', 'algorithmName', 'bestRun', 'cross_isi');
        
    else
        disp('which analysis MST');
        which_analysis
        size(data)
        ...
        if (~useTemporalICA)
            mygift_save(mstResultsFile, 'A', 'W', 'icasig', 'icasigR', 'algorithmName', 'corrMetric', 'bestRun');
        else
            temporal_icasig = icasig;
            clear icasig;
            mygift_save(mstResultsFile, 'A', 'W', 'temporal_icasig', 'icasigR', 'algorithmName', 'corrMetric', 'bestRun');
        end
    ...
    if (which_analysis == 1)
        % IVA
        disp('which analysis iva...');
        which_analysis
        size(data)

        [i, W] = mygift_icaAlgorithm(algorithmName, data, ICA_Options);
        ...
        if (which_analysis ~= 4)
            ...
            mygift_save(criResultsFile, 'A', 'W', 'icasig', 'algorithmName', 'cross_isi_joint', 'bestRun');
        end
...
if (strcmpi(modalityType, 'fmri'))
    if (~useTemporalICA)
        ...
        mygift_save(icaout, 'temporal_icasig');
        [A, W, icasig, temporal_icasig, skew] = backReconTemporalICAComps(sesInfo, A, temporal_icasig, compSetFields, dataMat);
    end
...
if (exist('skew', 'var'))
    mygift_save(icaout, 'W', 'icasig', 'mask_ind', 'skew');
else
    mygift_save(icaout, 'W', 'icasig', 'mask_ind');
end

if (exist('temporal_icasig', 'var'))
    mygift_save(icaout, 'temporal_icasig', '-append');
end
...
if (isempty(ica_fuse_findstr(lower(algorithmName),'iva')))
    ...
    mygift_save(icaout, 'A', '-append');
    
    % naming of complex images
    [sesInfo, complexInfo] = ica_fuse_name_complex_images(sesInfo, 'write');
    ...
    if strcmpi(modalityType, 'fmri')
        ...
        if(sesInfo.numOfSub ==1 & sesInfo.numOfSess==1)
            ...
            [aggFileNames, zipfilename, files_in_zip] = ica_fuse_saveICAData(outfile, icasig, A, mask_ind, numOfIC, sesInfo.HInfo, ...
                sesInfo.dataType, complexInfo, outputDir);
            drawnow;
        end
...
function [A, W, icasig, temporal_icasig, skew] = backReconTemporalICAComps(sesInfo, A, temporal_icasig, compSetFields, dataMat)
...
if ~isempty(sesInfo.inputFiles)
    ...
    icasig = icatb_parBackReconstruct(sesInfo, (1:length(sesInfo.inputFiles)), 1, dataMat);
else
    icasig = icatb_parBackReconstruct(sesInfo, (1:sesInfo.userInput.numOfSub), 1, dataMat);
end
...
for nF = 1:sesInfo.numOfSub   %%length(sesInfo.inputFiles)
    ...
    disp(msgString);
    drawnow;
    mygift_save(fullfile(outputDir, subFile), 'compSet');
...
for compNum = 1:numOfIC
    ...
    skew(compNum) = ica_fuse_skewness(v) + eps;
    clear v;
...
function  icasig = parICAReference(sesInfo, algorithmName, parallel_info, dataMat, statusHandle)
%% ICA with reference
%
...
if (strcmpi(parallelMode, 'serial'))
    %% Serial
    icatb_parICAReference(sesInfo, dataSetsToRun, algorithmName, dataMat, statusHandle);
else
    if (parallelCluster)
        %% Use PCT
        ...
        giftPath = fileparts(which('step_2_run_analysis.m'));
        dummyScriptPath = fullfile(giftPath, 'parallel_files', 'mygift_dummyScript.m');
        ...
        for nF = 1:num_workers
            ...
            commandStr = ['!matlab -nodesktop -nosplash -r "addpath(genpath(''', giftPath, '''));icatb_parICAReference(''', paramFile,  ''',[', ...
                num2str(tmpDataSetsToRun), '],''', algorithmName, dataMat, ''');exit" < "', dummyScriptPath, '" &'];
            eval(commandStr);
        end
...
mygift_save(fullfile(sesInfo.outputDir, [sesInfo.ica_mat_file, '.mat']), 'icasig');
