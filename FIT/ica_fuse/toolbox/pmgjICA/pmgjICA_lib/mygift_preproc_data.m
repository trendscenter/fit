function out = mygift_preproc_data(out, preProcType, verbose)
%% Pre-process data
%
% Inputs:
...
options = {'Remove Mean Per Timepoint', 'Remove Mean Per Voxel', 'Intensity Normalization', 'Variance Normalization', 'None'};
alias_options = {'rt', 'rv', 'in', 'vn', 'Nn'};
if (nargin == 0)
    out = options;
    return;
end
...
switch (preProcType)
    case {'remove mean per timepoint', 'rt'}
        ...
        %% Remove mean per time point or per subject
        out = mygift_remove_mean(out);
