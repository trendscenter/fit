function mygift_parameterErrorCheck(sesInfo, optional)
...
if ~isempty(sesInfo.inputFiles)
    ...
    numOfFileSets = length(sesInfo.inputFiles);  %%% 3 for the example
    %numOfScans = size(sesInfo.inputFiles(1).name,1);
    % get the number of time points
    [numOfScans] = icatb_get_countTimePoints(sesInfo.inputFiles(1));  %%% 220 for the examples
else
    numOfFileSets=sesInfo.userInput.numOfSub;
    numOfScans=sesInfo.userInput.numOffMRI;
end
...
algoStr = mygift_icaAlgorithm;
try
    algoType = deblank(algoStr(sesInfo.userInput.algorithm, :));
catch
    algoType = '';
end
