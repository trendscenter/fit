function [outputFiles] = ica_fuse_getOutputFileNames(prefix, numComp, numSub, numSess, writeInDirs, fileExtn)
...
[modalityType, dataTitle] = ica_fuse_get_modality;

if (~exist('fileExtn', 'var'))
...
if (~strcmpi(modalityType, 'smri'))
    ...
    if numSub*numSess == 1
        ...
        if strcmpi(fileExtn, '.img') || strcmpi(fileExtn, '.gii')
            ...
            for kk = 1:numComp
                % component index
                fileIndex = ica_fuse_returnFileIndex(kk);
    ...
    elseif numSub == 1 && numSess > 1
        ...
        if strcmpi(fileExtn, '.img') || strcmpi(fileExtn, '.gii')
            ...
            for ii = 1:numSub
                ...
                % subject index
                subIndex = ica_fuse_returnFileIndex(ii);
    ...
    if (strcmpi(fileExtn, '.nii'))
        ...
        for nComp = 1:numComp
            files{nComp} = [cNaming, ica_fuse_returnFileIndex(nComp), fileExtn];
        end
