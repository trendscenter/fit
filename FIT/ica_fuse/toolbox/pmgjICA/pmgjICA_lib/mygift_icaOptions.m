function ICA_Options = mygift_icaOptions(dataSize, algorithm_index, handle_visibility)
...
switch (lower(ica_algorithm))
    ...
    case {'infomax', 'sdd ica', 'pmljica'}
        ...
        inputText(numParameters).uiType = 'edit'; %%'popup';
        inputText(numParameters).answerString = num2str(0.0); %%{'0', '1'};
        inputText(numParameters).dataType = 'numeric';
        ...
        inputText(numParameters).promptString = ' Select Posact';  %%%% Need to see what it is?
        inputText(numParameters).uiType = 'edit'; %%'popup';
        inputText(numParameters).answerString = 'off'; %%{'off', 'on'};
        ...
        inputText(numParameters).uiType = 'edit';  %%'popup';
        inputText(numParameters).answerString = 'on'; %{'on', 'off'};
        inputText(numParameters).dataType = 'string';
    ...
    case 'sparse ica-ebm'
        ...
        dlg_title = 'Select the options for the sparse ICA-EBM algorithm';
        
        numParameters = 1;
        
        inputText(numParameters).promptString = 'Enter sparsity parameter (10^-6 - 1)';
        inputText(numParameters).uiType = 'edit';
        inputText(numParameters).answerString = '10^-4';
        inputText(numParameters).dataType = 'numeric';
        ...
        inputText(numParameters).promptString = 'Enter smoothness parameter (1-10^4)';
        inputText(numParameters).uiType = 'edit';
        inputText(numParameters).answerString = '10^2';
        inputText(numParameters).dataType = 'numeric';
        inputText(numParameters).tag = 'epsilon';
        inputText(numParameters).enable = 'on';
    ...
    case 'adaptive_constrained_ica_ebm'
        ...
        % dialog Title
        dlg_title = 'Select the Options for the adaptive constrained ICA EBM algorithm';
        
        numParameters = 1;
        
        inputText(numParameters).promptString = 'Select optimization approach';
        inputText(numParameters).uiType = 'popup';
        inputText(numParameters).answerString = char('newton', 'quasi');
        inputText(numParameters).dataType = 'string';
        ...
        inputText(numParameters).answerString = '';
        inputText(numParameters).dataType = 'string';
        inputText(numParameters).tag = 'W_init';
        inputText(numParameters).enable = 'on';
        ...
        inputText(numParameters).promptString = 'Select corr threshold for constrained algorithm';
        inputText(numParameters).uiType = 'edit';
        inputText(numParameters).answerString = '0.001:0.1:0.9';
        inputText(numParameters).dataType = 'numeric';
        inputText(numParameters).tag = 'rho_list';
        inputText(numParameters).enable = 'on';
...
%%%% Default
ICA_Options=cell(1, 2*length(inputText));
for i=1:length(inputText)
    ICA_Options{2*i - 1} = inputText(i).tag;
    dataType=inputText(i).dataType;
    
    if strcmp(lower(dataType), 'numeric')
        
        ICA_Options{2*i} = str2num(inputText(i).answerString);
    else
        ICA_Options{2*i} = inputText(i).answerString;
    end
end
